...
<head>
  ...
  <meta name="description" content="A complete AI coding agent powered by frontier models — DeepSeek V4, GPT-5.6 Luna, MiMo 2.5, MiniMax M3, Kimi K3, Claude Fable 5 — routed through cost-optimized inference providers. Daily reset quotas, zero code telemetry, and local checkout worldwide.">
  ...
  <style>
    ...
    .crypto-modal-card {
      background: #0d0d12; border: 1px solid var(--border-accent);
      border-radius: 8px; max-width: 540px; width: 100%; padding: 28px;
      box-shadow: 0 0 50px rgba(255, 176, 0, 0.2); position: relative;
    ...
    .crypto-modal-close { background: none; border: none; color: var(--paper-dim); font-size: 1.6rem; cursor: pointer; line-height: 1; padding: 4px; }
    .crypto-modal-close:hover { color: var(--amber); }
    .crypto-tabs { display: flex; gap: 6px; margin: 16px 0; overflow-x: auto; padding-bottom: 4px; }
    .crypto-tab {
      background: rgba(240, 237, 230, 0.05); border: 1px solid var(--border); color: var(--paper-dim);
      font-family: var(--font-mono); font-size: 0.72rem; font-weight: 700; padding: 8px 12px;
      cursor: pointer; border-radius: 4px; white-space: nowrap; transition: all 0.15s ease;
    }
    .crypto-tab:hover { border-color: var(--amber); color: var(--paper); }
    .crypto-tab.active { background: rgba(255, 176, 0, 0.12); border-color: var(--amber); color: var(--amber); }
    .crypto-address-box {
      background: #050507; border: 1px solid var(--border); border-radius: 6px; padding: 16px; margin-top: 10px;
    }
    /* ─── OAUTH WAITLIST — chamfer border fix (stroke follows clip-path) ─── */
    .oauth-grid {
      display: grid; grid-template-columns: 1fr 1fr; gap: 14px;
      margin: 2rem auto 1.1rem; max-width: 560px;
    }
    .oauth-wrap {
    ...
    .oauth-wrap:hover { background: var(--amber); transform: translateY(-1px); box-shadow: 0 6px 24px rgba(255,176,0,0.16); }
    .oauth-wrap:focus-within { background: var(--amber); }
    .btn-oauth-waitlist {
      display: flex; align-items: center; justify-content: center; gap: 10px;
      width: 100%; padding: 1rem 1.25rem; background: #14141a; color: #f0ede6;
      border: none; font-family: var(--font-mono); font-size: 0.83rem; font-weight: 800;
      letter-spacing: 0.02em; text-decoration: none; line-height: 1.2;
      clip-path: polygon(6px 0, 100% 0, 100% calc(100% - 6px), calc(100% - 6px) 100%, 0 100%, 0 6px);
      transition: color 0.15s ease, background 0.15s ease;
    }
    .btn-oauth-waitlist:hover { color: var(--amber); }
    .btn-oauth-waitlist svg { flex-shrink: 0; }
    .oauth-subtext {
      font-family: var(--font-mono); font-size: 0.70rem; letter-spacing: 0.06em;
      color: var(--paper-45); text-transform: uppercase; text-align: center;
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ─── WAITLIST & EARLY ACCESS — premium chamfer with real border ─── */
    .waitlist-outer {
      max-width: 680px; margin: 0 auto; padding: 1px;
      background: linear-gradient(180deg, rgba(255,176,0,0.45), rgba(255,176,0,0.18));
      clip-path: polygon(var(--chamfer) 0, 100% 0, 100% calc(100% - var(--chamfer)), calc(100% - var(--chamfer)) 100%, 0 100%, 0 var(--chamfer));
      box-shadow: 0 0 50px rgba(255,176,0,0.08), 0 20px 60px rgba(0,0,0,0.45);
    }
    .waitlist-card {
      background: radial-gradient(120% 120% at 0% 0%, rgba(255,176,0,0.07) 0%, transparent 45%), var(--bg);
    ...
    /* community funding card — chamfer, not rounded */
    .fund-card {
    ...
    @media (max-width: 600px) {
      ...
      .waitlist-outer { margin: 0 1rem; }
      .waitlist-card { padding: 2rem 1.4rem 1.6rem; }
    }
  ...
  <script>
    if (window.Paddle) {
      Paddle.Environment.set("${config.paddleEnv || 'sandbox'}");
...
<body>
  ...
  <section class="hero is-visible">
    <header class="site-header" role="banner">
      <nav class="site-nav" aria-label="Main navigation">
        ...
        <ul class="site-nav-links" id="nav-menu" role="list">
          ...
          <li>
            <a id="nav-join-btn" href="#early-access" class="btn-chamfer site-nav-cta btn-dark">
              Join Waitlist
            </a>
          </li>
        </ul>
    ...
    <div class="hero-content">
      ...
      <p class="hero-sub">
        A complete CLI coding agent powered by frontier models — DeepSeek V4, GPT-5.6 Luna, MiMo 2.5, MiniMax M3 — routed through cost-optimized providers. Generous daily reset quotas, transparent token economics, zero code telemetry, and local checkout rails worldwide.
      </p>
      <div class="hero-actions">
        <a href="#early-access" class="btn-chamfer btn-dark" id="hero-join-btn" style="padding:1rem 2.2rem;font-size:0.92rem;">
          Join Early Access Waitlist &rarr;
        </a>
        <button onclick="openCryptoModal()" class="btn-chamfer btn-amber" style="padding:1rem 1.8rem;font-size:0.88rem;border:none;cursor:pointer;">
          ⚡ Sponsor Public Utility
        </button>
        <a href="#intro-freeport" class="btn-chamfer btn-ghost">Our Mission</a>
      </div>
    </div>
    ...
    <div class="hero-ticker">
      <div class="hero-ticker-inner">
        <span>Zero telemetry</span> <span>•</span> <span>DeepSeek V4 · GPT-5.6 Luna · MiMo 2.5 · MiniMax M3</span> <span>•</span>
                ...
                <svg id="surveillance-eye" class="problem-eye-svg" viewBox="0 0 480 480" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g class="eye-linework" opacity="0.22">
                    ...
                    <g class="eye-blink-target">
                      ...
                      <circle id="eye-iris" cx="240" cy="240" r="35" stroke="#F0EDE6" stroke-width="1" fill="none"/>
                    </g>
                  ...
                  <!-- Pupil (wrapper translates toward cursor; glitch anim stays on the circle) -->
                  <g id="eye-pupil-follow">
        ...
        <div class="intro-flex">
          ...
          <div class="intro-diagram-wrap">
            <svg class="intro-diagram" viewBox="0 0 560 280" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <!-- Box: Open-weight models -->
              <g>
                <rect x="423" y="100" width="120" height="50" fill="rgba(10,10,10,0.95)" stroke="rgba(240,237,230,0.15)" stroke-width="1"/>
                <text x="483" y="121" text-anchor="middle" font-family="'Chivo Mono',monospace" font-size="7" letter-spacing="1.5" fill="rgba(240,237,230,0.4)">OPEN MODELS</text>
        ...
        <div class="three-col-grid">
          ...
          <div class="three-col-cell">
            <div class="three-col-inner">
              ...
              <p class="three-col-text three-col-text--dim">Frontier models — DeepSeek V4, GPT-5.6 Luna, MiMo 2.5, MiniMax M3 — stream first tokens in &lt;300ms via cost-optimized providers. Routing picks the cheapest healthy endpoint. Fully transparent pricing.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="how-section" id="our-approach">
      <div class="container">
        <div class="how-intro">
          ...
          <p class="how-intro-body">Freeport is a purpose-built system — device-code auth, open-model streaming, sponsor slot economics, and BYOK — engineered together for one job: real coding work, worldwide.</p>
        </div>
        ...
        <div class="how-stepper">
          <div class="how-steps-left">
            ...
            <button class="how-step" data-step="2" onclick="setHowStep(2,true)" aria-label="Step 2: Open-model stream">
              <div class="how-step-progress"><div class="how-step-progress-fill"></div></div>
              <div class="how-step-header">
                <div class="how-step-num">Step 02</div>
                <h3>Open-weight models stream at &lt;300ms first-token.</h3>
              </div>
              <div class="how-step-body"><div class="how-step-body-inner">
                <p>Sessions route across frontier models — DeepSeek V4, GPT-5.6 Luna, MiMo 2.5, MiniMax M3 — with automatic failover when an endpoint degrades. You see tokens streaming in real time at 118 tokens/sec average throughput.</p>
            ...
            <button class="how-step" data-step="3" onclick="setHowStep(3,true)" aria-label="Step 3: Sponsor slots fund the free tier">
              ...
              <div class="how-step-body"><div class="how-step-body-inner">
                <p>Free-tier sessions are funded by a single non-intrusive sponsor line shown at session start. Each impression earns ~$0.03 ad inventory value — enough to cover open-model inference cost per session.</p>
          ...
          <div class="how-illustration">
            <div class="shader-overlay-card" id="howCard">
              ...
              <div class="card-content active" data-content="1">
                ...
                <span class="hl-amber">✓</span> <span class="hl-bright">Code: FREE-PORT</span><br>
              ...
              <div class="card-content" data-content="2">
                ...
                <span class="hl-amber">Provider: DeepInfra, OpenRouter, Fireworks, CrofAI<br>
                <span class="hl-amber">Routing:</span> DeepSeek V4 · GPT-5.6 Luna · MiMo 2.5 · MiniMax M3<br>
    ...
    <section>
      <div class="stats-strip">
        ...
        <div class="stat-box">
          ...
          <span class="stat-note">Open-weight model stream</span>
        </div>
        <div class="stat-box">
        ...
        <div class="terminal-artifact">
          ...
          <div class="terminal-blueprint-left">
            <div class="bp-anno bp-at-34">
              <div class="bp-text">
                <div class="bp-title">Open-Weight Models</div>
 ...
 ╚═╝     ╚═╝  ╚═╝╚══════╝╚══════╝╚═╝      ╚═════╝ ╚═╝  ╚═╝   ╚═╝   </pre>
                  ...
                  <div class="term-line output">&gt; Code: <span class="success">FREE-PORT</span> · Browser opening...</div>
            ...
            <div class="monitor-bezel-bottom">
              ...
              <span class="model-tag">v2.4</span>
        ...
        <div class="terminal-annotations-grid">
          <div class="terminal-anno-item"><div class="terminal-anno-title">Open-Weight Models</div><div class="terminal-anno-desc">240ms first-token stream</div></div>
        ...
        <div class="pricing-grid">
          ...
          <div class="pricing-card">
            ...
            <ul class="pricing-features">
              ...
              <li>Powered by frontier models</li>
          ...
          <div class="pricing-card featured">
            <div class="pricing-label">Supporter</div>
            <h3>Pro</h3>
            <div class="pricing-price">$6</div>
            <div class="pricing-period">per month · via Paddle</div>
            <hr class="pricing-divider">
            <ul class="pricing-features">
              <li>6 sessions/day — your usage funds free access for everyone</li>
              <li>100% ad-free experience</li>
              <li>Priority routing on flagship open models</li>
    ...
    <!-- ═══ 11. EARLY ACCESS & WAITLIST — redesigned premium ═══ -->
    <section id="early-access" class="promise-section">
      <div class="container ea-container">
        ...
        <div class="waitlist-card">
          ...
          <!-- 1-Click OAuth Fast-Track Waitlist — fixed chamfer stroke -->
          <div class="oauth-grid">
            <div class="oauth-wrap">
              <a href="/api/auth/github/start" class="btn-oauth-waitlist">
                ...
                <span>Join Waitlist with <strong>GitHub</strong></span>
              </a>
            </div>
            <div class="oauth-wrap">
              <a href="/api/auth/google/start" class="btn-oauth-waitlist">
                ...
                <span>Join Waitlist with <strong>Google</strong></span>
              </a>
            </div>
          </div>
          <p class="oauth-subtext">✦ 1-click verification · Instant queue placement · No password required</p>

          <div class="waitlist-perks">
            <div class="waitlist-perk"><span style="color:var(--amber);">✦</span> 3 daily sessions (frontier models)</div>
            <div class="waitlist-perk"><span style="color:var(--amber);">✦</span> Zero code telemetry</div>
            <div class="waitlist-perk"><span style="color:var(--amber);">✦</span> Local terminal execution</div>
          </div>

          <!-- Community Funding & Sponsor Milestone Card — chamfer fixed -->
          <div class="fund-card">
            ...
            <p style="font-size:0.88rem;color:var(--paper-dim);line-height:1.5;margin-bottom:14px;">
              Freeport is an independent, developer-first public utility. Every community dollar directly funds open-model inference credits for developers in underserved regions.
            </p>
            <div style="display:flex;flex-wrap:wrap;gap:10px;">
              ...
              <button onclick="openCryptoModal()" class="btn-chamfer btn-outline-light" style="padding:0.6rem 1.2rem;font-size:0.75rem;border:1px solid var(--border);background:rgba(240,237,230,0.04);cursor:pointer;">
                ⚡ Donate Crypto (USDT / SOL / ETH)
              </button>
            </div>
           </div>
         </div>
      ...
      <div class="container">
        ...
        <div class="faq-list">
          ...
          <details class="faq-item">
            ...
            <div class="faq-answer-wrap"><div class="faq-answer"><p>Freeport routes across frontier models — DeepSeek V4, GPT-5.6 Luna, MiMo 2.5, MiniMax M3, Kimi K3, Claude Fable 5 — through cost-optimized providers, always picking the cheapest healthy endpoint. BYOK users can route directly to DeepSeek, Anthropic, or OpenAI with zero intermediary markup.</p></div></div>
          </details>
          <details class="faq-item">
            <summary class="faq-question">
            ...
            <div class="faq-answer-wrap"><div class="faq-answer"><p>The free tier provides 3 full sessions per day that reset at UTC midnight. Each session is funded by a single non-intrusive sponsor line shown at session start — earning ~$0.03 ad inventory value that covers the inference cost. Supporters (Pro subscribers) fund additional capacity so more people can code for free. No credit card required, ever.</p></div></div>
          </details>
          <details class="faq-item">
            <summary class="faq-question">
          ...
          <details class="faq-item">
            <summary class="faq-question">
              ...
              <span class="faq-question-text">What do Supporters get?</span>
              <span class="faq-chevron" aria-hidden="true"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></span>
            </summary>
            <div class="faq-answer-wrap"><div class="faq-answer"><p>Supporters (Pro subscribers at $6/mo) get 6 sessions per day, ad-free experience, priority routing, and the satisfaction of knowing their subscription directly funds free access for developers worldwide. Every supporter session subsidizes multiple free sessions — you are the reason this project stays accessible.</p></div></div>
          </details>
          </details>
          </details>
          <details class="faq-item">
          ...
          <details class="faq-item">
            ...
            <div class="faq-answer-wrap"><div class="faq-answer"><p>Freeport is released under the Apache-2.0 license. The server, CLI, and SDK are all open source. Unit economics (open-model rates, session cost caps, sponsor slot values) are validated in the test suite and visible in the admin dashboard.</p></div></div>
          </details>
        </div>
      </div>
    </section>
    ...
    <!-- ═══ 12b. SUPPORT FREEPORT ═══ -->
    <section style="padding: 6rem 0; text-align: center;">
      <div class="container" style="max-width: 600px;">
        <div class="badge" style="margin-bottom: 1.5rem;">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" stroke="var(--amber)" stroke-width="2" fill="none"></path></svg>
          Support the Launch
        </div>
        <h2 class="section-title" style="margin-bottom: 1rem;">Fund the launch.</h2>
        <p style="color: var(--paper-dim); font-family: var(--font-mono); font-size: 0.85rem; line-height: 1.6; margin-bottom: 2rem;">
          Freeport is self-funded. Every dollar goes directly to inference costs — keeping the service free and accessible worldwide. No VC, no bait-and-switch.
        </p>
        <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
          <button class="btn-chamfer btn-amber" onclick="openSupport()" style="font-size: 0.9rem;">
            Support Freeport
          </button>
          <a href="https://github.com/sponsors" target="_blank" class="btn-chamfer btn-outline-light" style="font-size: 0.9rem; text-decoration: none;">
            GitHub Sponsors
          </a>
        </div>
        <p style="color: var(--paper-dim); font-family: var(--font-mono); font-size: 0.7rem; margin-top: 1.5rem;">
          Paddle processes all payments · Works globally · Zero data telemetry
        </p>
      </div>
    </section>
  ...
  <section class="cta-section">
    ...
    <div class="container cta-container">
      ...
      <div class="cta-btn-wrap">
        <button class="btn-chamfer btn-amber cta-btn" onclick="document.getElementById('early-access').scrollIntoView({behavior:'smooth'})">
      ...
      <p class="cta-fine-print">Apache-2.0 · Transparent public utility powered by frontier models</p>
    </div>
  </section>
    ...
    <div class="container">
      <div class="footer-grid">
        ...
        <div class="footer-col">
          <h3>Product</h3>
          <ul>
            <li><a href="#early-access">Waitlist</a></li>
            <li><a href="#faq">FAQ</a></li>
            <li><a href="#terminal-demo">CLI Demo</a></li>
          </ul>
        </div>
        <div class="footer-col">
          <h3>Sponsor & Community</h3>
          <ul>
            <li><a href="https://github.com/sponsors/hazemmrad17" target="_blank" rel="noopener noreferrer">GitHub Sponsors</a></li>
            <li><a href="#early-access" onclick="openCryptoModal(event)">Crypto Donations</a></li>
            <li><a href="https://github.com/hazemmrad17/freeport" target="_blank" rel="noopener noreferrer">GitHub Repository</a></li>
          </ul>
        </div>
      </div>
      <div class="footer-bottom">
        ...
        <div class="footer-status">
          ...
          <span>Waitlist Active · $0 Infrastructure Gated</span>
        </div>
      </div>
    </div>
  </footer>
  ...
  <div id="crypto-modal" class="crypto-modal-backdrop" style="display:none;" onclick="closeCryptoModal(event)">
    <div class="crypto-modal-card" onclick="event.stopPropagation()">
      <div class="crypto-modal-header">
        <div style="display:flex;align-items:center;gap:8px;">
          <span class="badge">⚡ Direct Support</span>
          <span style="font-family:var(--font-mono);font-size:0.72rem;color:var(--amber);">$25 funds ~500 free sessions</span>
        </div>
        <button class="crypto-modal-close" onclick="closeCryptoModal()" aria-label="Close modal">&times;</button>
      </div>

      <h3 style="font-family:var(--font-display);font-size:1.35rem;font-weight:800;margin:12px 0 6px;">Support Freeport via Crypto</h3>
      <p style="color:var(--paper-dim);font-size:0.85rem;line-height:1.5;margin-bottom:14px;">
        Direct, borderless support with zero middleman fees. All contributions directly fund model inference pools for developers worldwide.
      </p>

      <!-- Network Tabs -->
      <div class="crypto-tabs">
        <button class="crypto-tab active" data-net="usdt" onclick="switchCryptoTab('usdt')">USDT (TRC-20)</button>
        <button class="crypto-tab" data-net="sol" onclick="switchCryptoTab('sol')">Solana (SOL/USDC)</button>
        <button class="crypto-tab" data-net="eth" onclick="switchCryptoTab('eth')">Ethereum (ETH/USDC)</button>
        <button class="crypto-tab" data-net="btc" onclick="switchCryptoTab('btc')">Bitcoin (BTC)</button>
      </div>

      <!-- Address Box -->
      <div class="crypto-address-box">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          <span id="crypto-network-label" class="mono-label">USDT (TRC-20 Network)</span>
          <span id="crypto-copy-hint" style="font-family:var(--font-mono);font-size:0.7rem;color:var(--amber);opacity:0;transition:opacity 0.2s;">✓ Copied to clipboard</span>
        </div>
        <div style="display:flex;gap:8px;align-items:center;">
          <input id="crypto-address-val" readonly value="${config.cryptoUsdtTrc20 || 'TRC20_WALLET_ADDRESS'}" style="background:#0a0a0c;border:1px solid rgba(240,237,230,0.15);color:var(--paper);font-family:var(--font-mono);font-size:0.78rem;padding:10px 12px;width:100%;border-radius:4px;outline:none;" />
          <button class="btn-chamfer btn-amber" onclick="copyCryptoAddress()" style="padding:10px 18px;font-size:0.75rem;white-space:nowrap;border:none;cursor:pointer;">Copy</button>
        </div>
      </div>

      <div style="margin-top:14px;font-family:var(--font-mono);font-size:0.7rem;color:var(--text-muted);display:flex;align-items:center;gap:6px;">
        <span>🔒 Verified Public Address</span>
        <span>•</span>
  ...
  <script>
    // ── Crypto Sponsorship Modal ──
    const cryptoAddresses = {
      usdt: '${config.cryptoUsdtTrc20 || "TRC20_ADDRESS_PENDING"}',
      sol: '${config.cryptoSolana || "SOL_ADDRESS_PENDING"}',
      eth: '${config.cryptoEthereum || "ETH_ADDRESS_PENDING"}',
      btc: '${config.cryptoBitcoin || "BTC_ADDRESS_PENDING"}'
    };

    const cryptoLabels = {
      usdt: 'USDT (TRC-20 Network)',
      sol: 'Solana (SOL / USDC on Solana)',
      eth: 'Ethereum (ETH / ERC-20 / EVM)',
      btc: 'Bitcoin (Native SegWit / Taproot)'
    };

    let activeCryptoNetwork = 'usdt';

    function openCryptoModal(e) {
      if (e) e.preventDefault();
      document.getElementById('crypto-modal').style.display = 'flex';
      switchCryptoTab('usdt');
    }

    // ── Support Freeport (one-time Paddle checkout) ──
    // TODO: Create a one-time product in Paddle dashboard:
    //   1. Go to Paddle → Catalog → Products → Add Product
    //   2. Name: "Support Freeport"
    //   3. Type: One-time
    //   4. Price: $10 (or flexible amounts)
    //   5. Paste the price ID below:
    const SUPPORT_PRICE_ID = "TODO_PADDLE_PRICE_ID";
    ...
    function openSupport() {
      ...
      if (SUPPORT_PRICE_ID === "TODO_PADDLE_PRICE_ID") {
        alert("Donation checkout not yet configured. Set SUPPORT_PRICE_ID in welcome.ts");
        return;
      }
      Paddle.Checkout.open({
        settings: {
          displayMode: "overlay",
          variant: "one-page",
          successUrl: window.location.origin + "/?supported=1",
        },
        items: [{ priceId: SUPPORT_PRICE_ID, quantity: 1 }],
      });
    }
    

    function closeCryptoModal(e) {
      if (e && e.target && e.target.id !== 'crypto-modal' && !e.target.classList.contains('crypto-modal-close')) return;
      document.getElementById('crypto-modal').style.display = 'none';
    }

    function switchCryptoTab(network) {
      activeCryptoNetwork = network;
      document.querySelectorAll('.crypto-tab').forEach(t => t.classList.remove('active'));
      const activeBtn = document.querySelector('.crypto-tab[data-net="' + network + '"]');
      if (activeBtn) activeBtn.classList.add('active');
      document.getElementById('crypto-network-label').textContent = cryptoLabels[network] || network.toUpperCase();
      document.getElementById('crypto-address-val').value = cryptoAddresses[network] || '';
      document.getElementById('crypto-copy-hint').style.opacity = '0';
    }

    function copyCryptoAddress() {
      const input = document.getElementById('crypto-address-val');
      if (!input) return;
      navigator.clipboard?.writeText(input.value).catch(() => {});
      const hint = document.getElementById('crypto-copy-hint');
      if (hint) {
    ...
    function setupEyeTracking() {
      const svg = document.getElementById('surveillance-eye');
      const pupilWrap = document.getElementById('eye-pupil-follow');
      const iris = document.getElementById('eye-iris');
      if (!svg || !pupilWrap) return;

      const MAX_X = 45, MAX_Y = 22;   // wander limits in viewBox units (socket ellipse)
      const REACH = 240;              // px distance at which the gaze saturates
      ...
      window.addEventListener('mousemove', (e) => {
        ...
        const pull = Math.min(dist / REACH, 1);
        targetX = (dx / dist) * MAX_X * pull;
        targetY = (dy / dist) * MAX_Y * pull;
      }, { passive: true });

      (function follow() {
        curX += (targetX - curX) * 0.12;
        curY += (targetY - curY) * 0.12;
        pupilWrap.setAttribute('transform', 'translate(' + curX.toFixed(2) + ' ' + curY.toFixed(2) + ')');
        if (iris) iris.setAttribute('transform', 'translate(' + (curX * 0.55).toFixed(2) + ' ' + (curY * 0.55).toFixed(2) + ')');
