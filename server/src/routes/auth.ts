...
authRoutes.get('/api/auth/github/callback', async (c) => {
  ...
  try {
    ...
    const waitlistResult = joinWaitlist(db, user.email, `oauth_${user.auth_provider}`)

    if (attempt) {
      const { token } = createApiToken(db, user.id)
      approveAttempt(db, attempt.id, user.id, token)
      return c.html(successPageHtml(device, user, waitlistResult))
    }

    return c.html(successPageHtml('', user, waitlistResult))
  } catch (err) {
...
function successPageHtml(
  deviceCode: string,
  user?: UserRow,
  waitlist?: { position: number; alreadyJoined: boolean },
): string {
  ...
  const trustLabel =
    user?.trust_tier === 'trusted'
      ? 'Trusted Tier (Priority Queue)'
      : user?.trust_tier === 'low'
        ? 'Standard Tier'
        : 'Verified Developer'

  const isWaitlistWeb = !deviceCode

  return `<!doctype html><html lang="en"><head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${isWaitlistWeb ? 'Early Access Confirmed — Freeport' : 'Signed in — Freeport'}</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
...
<body>
  ...
  <div class="auth-wrap">
    ...
    <div class="auth-card" style="text-align: center;">
      ...
      <div class="ok"><strong>${
        isWaitlistWeb ? 'Spot Reserved on Waitlist!' : "You're signed in."
      }</strong> ${
        deviceCode
          ? `Code <code>${deviceCode}</code> approved.`
          : `Verified via ${user?.auth_provider?.toUpperCase() ?? 'OAuth'}.`
      }</div>

      <h1 style="margin-bottom: 8px;">${
        isWaitlistWeb ? `You're #${waitlist?.position ?? 1} in Queue` : 'Terminal Authorized'
      }</h1>
      
      <p class="auth-desc" style="margin-bottom: 16px;">${
        isWaitlistWeb
          ? `We've linked your verified <strong>${user?.email}</strong> profile. As community funding milestones unlock daily batches, you will receive activation instructions directly.`
          : 'You can now close this browser tab and return to your terminal to start using Freeport.'
      }</p>
      ...
      ${
        isWaitlistWeb
          ? `<div style="background: rgba(255,176,0,0.06); border: 1px solid rgba(255,176,0,0.3); border-radius: 4px; padding: 14px; margin-top: 15px; font-family: var(--font-mono); font-size: 0.8rem; color: var(--paper);">
              ✦ Priority Developer Queue · Zero Code Telemetry
            </div>`
      ...
      <a href="/" class="btn-chamfer" style="margin-top: 20px;">Return to Freeport Home</a>
    </div>
  </div>
</body></html>`
}
