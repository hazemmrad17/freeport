...
/**
 * Commands that should be REMOVED in FREEPORT.
 * These are stripped at build time via the FREEPORT_REMOVED_COMMAND_IDS set
...
/**
 * Commands that should be KEPT in FREEPORT.
...
describe.skip('FREEPORT: Slash Commands', () => {
  ...
  test(
    ...
    async () => {
      ...
      session = await FREEPORTSession.start(binary, { waitSeconds: 5, height: SESSION_HEIGHT })
  ...
  test(
    ...
    async () => {
      ...
      // Mode commands should not exist in FREEPORT
