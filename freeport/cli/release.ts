...
/**
 * FREEPORT CLI release script.
 *
 * Triggers the FREEPORT-release.yml GitHub Actions workflow
 * to build, publish, and release the FREEPORT CLI to npm.
 *
 * Usage:
 *   bun FREEPORT/cli/release.ts [patch|minor|major] [--ref <commit-sha>]
 *
 * Requires:
...
async function triggerWorkflow(versionType: string, checkoutRef: string) {
  ...
  try {
    ...
    const triggerCmd = `curl -s -w "HTTP Status: %{http_code}" -X POST \
      ...
      https://api.github.com/repos/CodebuffAI/FREEPORT-private/actions/workflows/FREEPORT-release.yml/dispatches \
      -d '${payload}'`
    ...
    if (response.includes('workflow_dispatch')) {
      ...
      log(
        'Please manually trigger the workflow at: https://github.com/CodebuffAI/FREEPORT-private/actions/workflows/FREEPORT-release.yml',
      )
    } else {
      log('🎉 FREEPORT release workflow triggered!')
    }
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err)
    log(`⚠️  Failed to trigger workflow automatically: ${message}`)
    log(
      'You may need to trigger it manually at: https://github.com/CodebuffAI/FREEPORT-private/actions/workflows/FREEPORT-release.yml',
    )
  }
}

async function main() {
  log('🚀 Initiating FREEPORT release...')
  ...
  log('')
  log(
    'Monitor progress at: https://github.com/CodebuffAI/FREEPORT-private/actions/workflows/FREEPORT-release.yml',
  )
}
