...
//
//   - "will run commands on your behalf" — codebuff/FREEPORT main surface
//     header (authed + session ready)
//   - "Press ENTER to login" / "Open this URL" — login modal (no cached
//     creds — typical CI smoke)
//   - "Pick a model to start" — FREEPORT model-picker landing screen
//   - "Free mode isn't available" — FREEPORT country-block screen (CI
...
// fires after the renderer is up). The wasm-load rejection on FREEPORT
...
// what tripped FREEPORT 0.0.62 in the wild while a 5s window let CI pass.
