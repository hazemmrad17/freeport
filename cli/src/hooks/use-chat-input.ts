...
export const useChatInput = ({
  ...
  // In FREEPORT, the toggle is always hidden, so never reserve width for it.
  // In non-FREEPORT: hide in bash mode, compact height, or narrow width.
  const estimatedToggleWidth =
    IS_FREEPORT || inputMode !== 'default' || isCompactHeight || isNarrowWidth
      ? 0
