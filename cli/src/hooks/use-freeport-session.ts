...
import {
  getSelectedfreeportModel,
  usefreeportModelStore,
} from '../state/freeport-model-store'
...
import {
  ...
  getCachedReferral,
  rememberReferral,
} from '../utils/freeport-referral-cache'
import {
  callfreeportSession,
  classifyfreeportSessionRequestFailure,
  freeportSessionRequestError,
  holdsLivefreeportSlot,
  isfreeportSessionTimeoutError,
...
import { savefreeportModelPreference } from '../utils/settings'
...
const playAdmissionSound = () => {
  try {
    ...
    // Silent fallback ΓÇö some terminals/pipes disallow writing to stdout.
  }
}

/** Picks the poll delay after a successful tick. Returns null when the state
 *  is terminal (no further polling). */
function nextDelayMs(next: freeportSessionResponse): number | null {
 ...
 *   - 'rejoin'  ΓåÆ POST: claim/rotate a seat (used after explicit end-and-rejoin
 *                 or when the chat gate kicks us back to the queue).
 *   - 'landing' ΓåÆ GET: drop to the model-picker (status 'none') so the user
...
interface PollController {
  ...
  apply: (next: freeportSessionResponse) => void
  abort: () => void
}
...
/**
 * The model of the most recent EXPLICIT user pick (startfreeportSession),
 ...
 *  `holdsLivefreeportSlot` so the two can't drift: an id exists exactly while
 *  we hold a live slot (active, or `ended` inside the server-side grace
 *  window where the row stays alive until `expires_at + grace`). */
export function getfreeportInstanceId(): string | undefined {
  const current = usefreeportSessionStore.getState().session
  if (!current || !holdsLivefreeportSlot(current)) return undefined
 ...
 *  instance id). Chat requests are only admissible in these states ΓÇö once
 *  the slot is gone, `getfreeportInstanceId` returns undefined and the
 *  server rejects the request ΓÇö so the message queue gates on this before
...
function toLandingSession(
  current: freeportSessionResponse | null,
): Extract<freeportSessionResponse, { status: 'none' }> {
  const accessTier =
...
async function restartfreeportSession(
  mode: RestartMode,
  opts: RestartOpts = {},
): Promise<void> {
  if (!IS_FREEPORT) return
  ...
  controller?.abort()
  if (opts.releaseSlot) await releasefreeportSlot()
  await controller?.restart(mode)
}

/**
 * Re-POST to the server (rejoining the queue / rotating the instance id).
 * Pass `resetChat: true` to also wipe local chat history ΓÇö used when
 * rejoining after a session ended so the next admitted session starts fresh.
 */
export function refreshfreeportSession(
  opts: { resetChat?: boolean } = {},
): Promise<void> {
  return restartfreeportSession('rejoin', { resetChat: opts.resetChat })
}
 ...
 */
export function returnTofreeportLanding(
  opts: { resetChat?: boolean } = {},
): Promise<void> {
  return restartfreeportSession('landing', {
...
export function refreshfreeportLandingMetadata(): Promise<void> {
  return restartfreeportSession('landing')
}
 ...
 */
export function startfreeportSession(model: string): Promise<void> {
  if (!IS_FREEPORT) return Promise.resolve()
  // This is the only explicit user-pick path (called from the picker on
  // click / Enter), so persistence belongs here ΓÇö and ONLY here. Server-
  ...
  const resolved = resolvefreeportModelForAccessTier(model, accessTier)
  ...
  usefreeportModelStore.getState().setSelectedModel(resolved)
  savefreeportModelPreference(resolved)
  return restartfreeportSession('rejoin')
}

let takeoverInFlight: Promise<void> | null = null

export function takeOverfreeportSession(): Promise<void> {
  if (!IS_FREEPORT) return Promise.resolve()
  if (takeoverInFlight) return takeoverInFlight

  const { session } = usefreeportSessionStore.getState()
  if (session?.status !== 'takeover_prompt') {
    return Promise.resolve()
  }

  usefreeportModelStore.getState().setSelectedModel(session.model)
  takeoverInFlight = restartfreeportSession('rejoin').finally(() => {
    takeoverInFlight = null
  })
  return takeoverInFlight
}

export function markfreeportSessionSuperseded(): void {
  if (!IS_FREEPORT) return
  controller?.abort()
...
export function markfreeportSessionCountryBlocked(params: {
  countryCode: string
...
export function markfreeportSessionEnded(): void {
  if (!IS_FREEPORT) return
  controller?.abort()
  const current = usefreeportSessionStore.getState().session
  const rateLimitsByModel = getRateLimitsByModel(current)
...
interface UsefreeportSessionResult {
  session: freeportSessionResponse | null
  failure: ReturnType<typeof usefreeportSessionStore.getState>['failure']
}

/**
 * Manages the FREEPORT session lifecycle:
 *   - GET on mount to probe state (no auto-join; the user picks a model in
 *     the landing screen, which calls startfreeportSession)
 ...
 */
export function usefreeportSession(): UsefreeportSessionResult {
  const session = usefreeportSessionStore((s) => s.session)
  const failure = usefreeportSessionStore((s) => s.failure)

  useEffect(() => {
    const { setSession, setFailure } = usefreeportSessionStore.getState()

    if (!IS_FREEPORT) {
      // Non-FREEPORT (Codebuff) builds never gate on a free session; leave the
      // store empty (app.tsx's session routing is all behind IS_FREEPORT).
      setSession(null)
      return
    }
    ...
    let previousStatus: freeportSessionResponse['status'] | null = null
    ...
    const apply = (next: freeportSessionResponse) => {
      ...
      if (next.status === 'active') {
        ...
        usefreeportModelStore
          .getState()
          .setSelectedModel(LIMITED_FREEPORT_MODEL_ID)
      }
      setSession(next)
      setFailure(null)
    ...
    const tick = async () => {
      ...
      const instanceId = getfreeportInstanceId()
      const model = getSelectedfreeportModel()
      const compact =
        method === 'GET' && previousStatus === 'active' && !needsFullActivePoll
      const fetchController = abortController
      const generation = restartGeneration
      try {
        const next = await callfreeportSession(method, token, {
        ...
        //     click ΓÇö end the locked session (usually a stale row from a
        ...
        if (next.status === 'model_locked') {
          if (explicitPickModel && explicitPickModel !== next.currentModel) {
            const current = getfreeportModel(next.currentModel).displayName
            const requested = getfreeportModel(explicitPickModel).displayName
            let released = false
            try {
              await callfreeportSession('DELETE', token, {
                signal: fetchController.signal,
              })
              released = true
            } catch {
              // DELETE failed ΓÇö fall through to the revert-with-explanation
          ...
          usefreeportModelStore.getState().setSelectedModel(next.currentModel)
          schedule(0)
          return
        }
        if (next.status === 'model_unavailable') {
          // Server says the requested model isn't available right now. Flip
          // to the always-available fallback for this run. In-memory only ΓÇö
          ...
          // would otherwise land on a different model with no explanation ΓÇö
          // they lost a race for the wave's last slot.
          if (isfreeportLimitedOfferModelId(next.requestedModel)) {
            const requested = getfreeportModel(next.requestedModel).displayName
            const fallback = getfreeportModel(
              FALLBACK_FREEPORT_MODEL_ID,
            ).displayName
            useChatStore
              .getState()
              .setMessages((prev) => [
                ...prev,
                getSystemMessage(
                  `${requested}'s trial sessions just ran out, so this session started on ${fallback} instead. Check back later ΓÇö we release more in batches.`,
                ),
              ])
          }
          usefreeportModelStore
        ...
        // rotate our instance id; otherwise opening a second FREEPORT would
        ...
        // active|ended ΓåÆ none means we've passed the server's hard cutoff.
        ...
        // snapshot we have ΓÇö preferring the fresh `none` snapshot, falling
        // back to whatever was on the prior active/ended row ΓÇö so the
        ...
        if (compact && next.status === 'active') {
          const merged = mergeCompactActiveSession(
            usefreeportSessionStore.getState().session,
            next,
          )
        ...
        consecutiveFailures++
        const disposition = classifyfreeportSessionRequestFailure(method, err)
        const shouldRetry = disposition === 'retry'
        const retryAfterMs =
          err instanceof freeportSessionRequestError
            ? err.retryAfterMs
            : undefined
        ...
        logger.warn(
          ...
          shouldRetry
            ? '[freeport-session] fetch failed; backing off'
            : '[freeport-session] fetch failed; automatic retry stopped',
        )
        const retry =
          delayMs === null
            ? null
        ...
        if (err instanceof freeportSessionRequestError) {
          setFailure({
            ...failure,
            type: 'http',
            statusCode: err.statusCode,
          })
        } else if (isfreeportSessionTimeoutError(err)) {
    ...
    controller = {
      restart: async (mode) => {
        ...
        // a forced restart, and so the active|ended ΓåÆ none synthesis below
        ...
        if (mode === 'landing') {
          ...
          // auto-POST ΓÇö the exact silent-rejoin this mode exists to
          ...
          // when the user commits to a model via startfreeportSession.
          const landingSession = toLandingSession(
            usefreeportSessionStore.getState().session,
          )
          apply(landingSession)
          const fetchController = abortController
          callfreeportSession('GET', token, {
            signal: fetchController.signal,
          })
            .then((response) => {
              if (
            ...
            .catch(() => {
              // Silent ΓÇö blank hints are acceptable if the fetch fails.
            })
          return
        }
        nextMethod = 'POST'
    ...
    return () => {
      ...
      if (holdsLivefreeportSlot(current)) {
        callfreeportSession('DELETE', token).catch(() => {})
      }
      setSession(null)
      setFailure(null)
    }
