...
const activeSession: freeportSessionResponse = {
  status: 'active',
  accessTier: 'full',
  instanceId: 'inst-1',
...
describe('holdsLivefreeportSlot', () => {
  test('active session holds a slot', () => {
    expect(holdsLivefreeportSlot(activeSession)).toBe(true)
  })

  test('ended WITH instance id (grace window) still holds a slot', () => {
    expect(
      holdsLivefreeportSlot({ status: 'ended', instanceId: 'inst-1' }),
    ).toBe(true)
  })

  test('ended WITHOUT instance id (post-grace) does not hold a slot', () => {
    expect(holdsLivefreeportSlot({ status: 'ended' })).toBe(false)
  })

  test('null session does not hold a slot', () => {
    expect(holdsLivefreeportSlot(null)).toBe(false)
  })

  test('pre-join and terminal states do not hold a slot', () => {
    expect(holdsLivefreeportSlot({ status: 'none' })).toBe(false)
    expect(holdsLivefreeportSlot({ status: 'superseded' })).toBe(false)
    expect(
      holdsLivefreeportSlot({ status: 'takeover_prompt', model: 'm' }),
    ).toBe(false)
  })
})
