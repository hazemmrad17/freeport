...
import { startfreeportSession } from '../hooks/use-freeport-session'
...
import {
  formatfreeportPremiumResetCountdown,
  getfreeportPremiumResetAt,
} from '../utils/freeport-premium-reset'
...
import type { freeportReferralFocusTarget } from './freeport-referral-banner'
import type {
  BoxRenderable,
  KeyEvent,
  ScrollBoxRenderable,
...
// inline ΓÇö "N of M used ┬╖ resets in ΓÇª" ΓÇö once any session is spent (turning
...
// again (DEFAULT_FREEPORT_MODEL_ID), so it draws on that same pool and flips to
// the unlimited Flash once the pool empties ΓÇö the hero must always be joinable.
...
type Section = {
  ...
  label: string
  models: readonly freeportModelOption[]
}
...
const TOGGLE_ID = '__FREEPORT_toggle__'

// There used to be a right-aligned "Press Enter Γå╡" cue on the focused row, with
 ...
 * Picking a model is their explicit commitment to enter ΓÇö this triggers the
 ...
 * Space) commits the focused row ΓÇö or, on the toggle, expands/collapses the
 ...
 * deployment hours) always sit on their own centered line under the name ΓÇö
...
function gridModels(
  ...
  return getfreeportModelsForAccessTier(accessTier).filter(
    (m) => !isfreeportGlmV52ModelId(m.id),
  )
}

/** Every model id this screen can offer a tier: the grid, plus the banner's earned GLM
 *  action. Exported so the offerΓåÆgate invariant test reads the real set rather than a
 *  copy of it.
 *
 ...
 *  worth the same in every region. The balance is what gates the row ΓÇö the banner only
 *  renders the action when the server reports sessions left ΓÇö and the tier never was. */
export function freeportCliOfferedModelIds(
  accessTier: freeportAccessTier,
): readonly string[] {
...
export const freeportModelSelector: React.FC<freeportModelSelectorProps> = ({
  ...
  // contentMaxWidth (not terminalWidth) is the real budget ΓÇö the parent
  ...
  const setSelectedModel = usefreeportModelStore((s) => s.setSelectedModel)
  const session = usefreeportSessionStore((s) => s.session)
  const accessTier =
    (session && 'accessTier' in session ? session.accessTier : undefined) ??
    'full'
  const now = useNow(60_000)
  const deploymentAvailabilityLabel = useMemo(
    () => getfreeportDeploymentAvailabilityLabel(new Date(now)),
    [now],
  )
  ...
  // it rendered before the offer existed ΓÇö no stale row, no greyed-out tease.
  //
  ...
  const offers = useMemo(
    () =>
      getLimitedModelOffers(session).filter((offer) =>
        isSupportedfreeportModelId(offer.model),
      ),
    [session],
  )
  const offerModels = useMemo(
    () => offers.map((offer) => getfreeportModel(offer.model)),
    [offers],
  )
  ...
  // ┬╖ resets in ΓÇª". All premium models share one pool; the server replicates
  // the same snapshot under every model id, so any entry has the right count.
  // The count shows from the start ΓÇö even at "0 of M" ΓÇö so full-access users
  ...
  // the recommendation is premium) ΓÇö the hero must always be joinable. (The PREMIUM
  ...
  //
  // getfreeportSectionQuotas decides by counting rows, so nothing here knows
  // WHICH model is the odd one ΓÇö the next per-model ceiling is a server change
  // that this build renders without being rebuilt.
  const premiumSectionQuotas = getfreeportSectionQuotas(
    availableModels
      .filter((m) => isfreeportPremiumModelId(m.id))
      .map((m) => m.id),
    rateLimitsByModel,
  )
  const sharedRateLimit = premiumSectionQuotas.header
  const premiumUsed = sharedRateLimit?.recentCount ?? 0
  const premiumLimit = sharedRateLimit?.limit ?? FREEPORT_PREMIUM_SESSION_LIMIT
  ...
  // countdown is meaningful even at zero used ΓÇö getfreeportPremiumResetAt falls
  // back to the next day boundary when the server hasn't sent a resetAt yet.
  const premiumResetCountdown = formatfreeportPremiumResetCountdown(
    getfreeportPremiumResetAt({ rateLimitsByModel, nowMs: now }),
    now,
  )

  const recommendedModel = useMemo(() => {
    const id = getRecommendedfreeportModelId(accessTier, { premiumExhausted })
  ...
  // switch with, so it shows the notice only ΓÇö the replacement is always
  // reachable as a row in this same picker (and is usually the RECOMMENDED hero
  // one Enter away), which is what getfreeportModelSupersededBy guarantees by
  // resolving against the models actually on screen.
  const supersededNoticeFor = useCallback(
    (model: freeportModelOption): string | undefined =>
      // Only on the row the user is actually on ΓÇö the nudge is about THEIR
  ...
  // single "other" model (limited tier) we just show both ΓÇö a "see 1 more
  // model" toggle is noise.
  const canCollapse = otherModels.length >= 2

  const isJoinable = useCallback(
    (modelId: string) => {
      if (!isfreeportModelAvailable(modelId, new Date(now))) return false
  ...
  // selection IS the recommended model ΓÇö a returning user whose preference is a
  ...
  // "focused" buries the hero under rows the user cannot press ΓÇö which is what
  ...
  //
  // Mirror the settled state up to the landing screen (collapsed ΓåÆ it promotes
  ...
  // Keyboard cursor ΓÇö separate from the actually-selected model so that
      ...
      [
        {
          key: 'premium',
          label: 'PREMIUM',
          models: availableModels.filter((m) => isfreeportPremiumModelId(m.id)),
        },
        {
          key: 'unlimited',
          label: 'UNLIMITED',
          models: availableModels.filter(
            (m) => !isfreeportPremiumModelId(m.id),
          ),
        },
      ] satisfies readonly Section[]
  ...
  // render, the navigation order and the height estimate ΓÇö those three must
  ...
  // on ΓÇö hiding it behind "see all models" would mean most users never learn
  ...
  // models" reaches its buttons; nextfreeportModelId wraps back to the top).
  ...
  useEffect(() => {
    ...
    // actually start, or Enter POSTs a model the server rejects ΓÇö or, worse,
    ...
    //    selectable from the banner but is not in FREEPORT_MODELS, so it never
    ...
    //    and silently swapped for the recommendation ΓÇö the bounty session they
    ...
    //
    // In-memory only ΓÇö `setSelectedModel` doesn't persist, so the user's saved
    // preference survives for their next launch.
    const selectionIsStartable = isfreeportGlmV52ModelId(selectedModel)
  ...
  // The warning ALWAYS gets its own line rather than being appended to line 1 ΓÇö
  ...
  const { compactNames, buttonOuterWidth, buttonInnerWidth, nameColumnWidth } =
    useMemo(() => {
      ...
      const detailsParts = (model: freeportModelOption): number[] => {
        const parts: number[] = []
      ...
      const noticeLineLen = (m: freeportModelOption) =>
      ...
      const compactLabelLen = (m: freeportModelOption) =>
        2 +
      ...
      // card rather than indented under line 1's details column ΓÇö the notice is
      ...
      // stretches the card.
      const detailsLineLen = (m: freeportModelOption) =>
        joinedLen(detailsParts(m))

      // Cards are exactly as wide as their widest line. Nothing is reserved
      // beyond that ΓÇö the removed "Press Enter Γå╡" gutter used to pad every card
      // out to the hero's line plus 17 columns of empty space.
      const innerWidth = (labelLen: (m: freeportModelOption) => number) =>
        Math.max(
          ...widthModels.map((m) =>
  ...
  const estimatedModelHeight = useMemo(() => {
    let y = 0
    const rowHeight = (m: freeportModelOption) =>
  ...
  const pick = useCallback(
    (modelId: string) => {
      ...
      setPending(modelId)
      startfreeportSession(modelId).finally(() => setPending(null))
    },
    ...
    useCallback(
      (key: KeyEvent) => {
        ...
        if (isCommit) {
          ...
          // A referral-banner button (copy invite link / use GLM) is focused ΓÇö
  ...
  const renderModelButton = (
    model: freeportModelOption,
    options: { recommended?: boolean } = {},
  ) => {
    // Single visual state: the focused row IS the highlight. The user's
    // saved/committed pick is not shown separately ΓÇö it just sets where
    ...
    // Clickable whenever picking would actually do something ΓÇö i.e.
    ...
    // itself stays the normal foreground color so it doesn't shout ΓÇö the
    // border and arrow do the highlighting. Off-focus rows are default.
    const indicator = isFocused ? 'ΓÇ║' : ' '
    const fgColor = canJoin ? theme.foreground : theme.muted
    const mutedColor = theme.muted
    const warningColor = theme.secondary

    // Focused row gets the bright primary border (and arrow). Every other row ΓÇö
    // including the recommended card when the cursor has moved elsewhere ΓÇö stays
    ...
    // while closed (the label flips inside getfreeportDeploymentAvailabilityLabel),
    ...
    // Any OTHER reason a row is closed ΓÇö V4 Pro's peak window today ΓÇö says when
    ...
    // the outside.
    const unavailableLabel = getfreeportModelUnavailableLabel(
      model.id,
      new Date(now),
    )
    ...
    // meaningless ΓÇö and it is what stretched line 1 on rows that can't actually
    // see pixels.
    const imagesSuffix = model.multimodal ? ' ┬╖ Images' : ''

    // The effort the server runs this model at (the same catalog field the
    // completions layer sends) ΓÇö see freeportModelOption.reasoningEffort.
    ...
    const ownQuotaLabel = ownQuota ? formatfreeportRowQuota(ownQuota) : null
  ...
  // Scarcity, on the LIMITED TRIAL header rather than on the row ΓÇö same
  ...
  // theirs) when they personally get another. `offers` is homogeneous ΓÇö one
  // pool, one per-user ceiling ΓÇö so the first entry speaks for all of them.
  ...
  const offerUserResetCountdown =
    offerUserResetAt && Number.isFinite(offerUserResetAt.getTime())
      ? formatfreeportPremiumResetCountdown(offerUserResetAt, now)
      : null
  ...
  const sectionsContent = renderedSections.map((section) => (
    ...
    >
      {/* wrapMode 'none' pins headers to one row ΓÇö the offset math above
  ...
  const toggleLabel = expanded
    ? 'Γåæ  Show fewer'
    : `Γåô  See all ${availableModels.length} models`
