...
import {
  FREEPORT_PAUSED_MODEL_NOTICE,
  FREEPORT_TIER_CHANGE_NOTICE,
  getfreeportModelAvailabilityNotice,
} from '@codebuff/common/util/freeport-model-availability'
import { formatfreeportHardBlockedPrivacySignals } from '@codebuff/common/util/freeport-privacy'

import type { freeportStreakLine } from '../utils/freeport-streak-line'
import type { freeportSessionFailure } from '../state/freeport-session-store'
...
// Rendered directly under the model list ΓÇö that's where "why these models?"
// gets asked. The copy itself is shared with FREEPORT Desktop's model menu; see
// `getfreeportModelAvailabilityNotice` for the tone rules it follows.
...
function getTakeoverErrorMessage(failure: freeportSessionFailure): string {
  if (failure.type === 'http' && failure.statusCode === 503) {
    return "FREEPORT is busy and couldn't complete the takeover yet."
  }
  if (failure.type === 'timeout') {
    return failure.outcomeUnknown
      ...
      : failure.retry
        ? 'The takeover request timed out while FREEPORT was busy.'
        : 'The takeover request timed out.'
  }
  if (failure.outcomeUnknown) {
    return "FREEPORT couldn't confirm whether the takeover succeeded. Check the warning, then retry if you still want to take over."
  }
...
}> = ({ failure, onTakeOver = takeOverfreeportSession }) => {
  ...
  return (
    ...
    >
      <text style={{ fg: theme.foreground }} attributes={TextAttributes.BOLD}>
        FREEPORT is already running
      </text>

      <text style={{ fg: theme.muted }}>
        Only one FREEPORT instance is allowed at a time.
      </text>
        ...
        >
          <text
            style={{
              ...
              // text ΓÇö on the green fill it renders the label invisible.
...
const streakSpans = (
  ...
  <span key="dots" fg={theme.primary}>
    {`${' '.repeat(FREEPORT_STREAK_LABEL_GAP)}${line.dots}`}
  </span>,
]
 ...
 *  one (only when it fits ΓÇö see fitsfreeportStreakOnHeadingRow).
 *
 ...
 *  container segfaults the renderer ΓÇö reachable by dragging the terminal
...
export const LandingHeadingRow: React.FC<{
  streakLine: freeportStreakLine | null
  marginBottom: number
  ...
  return (
    <text style={{ marginBottom, wrapMode: 'word' }}>
      ...
      {streakLine && [
        <span key="gap">{' '.repeat(FREEPORT_STREAK_INLINE_GAP)}</span>,
...
export const freeportLandingScreen: React.FC<freeportLandingScreenProps> = ({
  ...
  //   shorter      : no logo ΓÇö the heading already identifies the experience
  ...
  // Section headers always show ΓÇö the picker scrolls within whatever rows
  ...
  // referral-free picker we still show the full ASCII logo ΓÇö it fills what
  ...
  // Always enable ads on the landing screen ΓÇö this is where monetization lives.
  ...
  const { ads, recordClick, recordImpression } = useGravityAd({
    ...
    forceStart: true,
    provider: 'gravity',
    // Legacy wire name for this surface ΓÇö the ads API maps it to placements,
  ...
  //
  // Never both ΓÇö a limited-tier user has neither Pro nor a premium pool, so the
  ...
  const belowPickerNotices = compact
    ? []
    : accessTier === 'limited'
      ? [getLimitedModeNotice(session), FREEPORT_PAUSED_MODEL_NOTICE]
      : [FREEPORT_TIER_CHANGE_NOTICE]
  ...
  // startfreeportSession, which POSTs and transitions straight to 'active' (chat).
  const isLanding = session?.status === 'none'
  const streakQuery = usefreeportStreakQuery({
    enabled: FREEPORT_ENABLE_STREAK_IN_UI && isLanding,
  })
  ...
  const showStreakIndicator = FREEPORT_ENABLE_STREAK_IN_UI && isLanding
  ...
  const streakLine = showStreakIndicator ? getfreeportStreakLine(streak) : null
  ...
  // Hide the "0 of N ΓÇª used" line entirely for a fresh user ΓÇö a zeroed counter
  ...
  // (getRecommendedfreeportModelId, while the pool has sessions left) the count
  ...
  const sessionResetAt = getfreeportPremiumResetAt({
    rateLimitsByModel,
    nowMs: now,
  })
  const sessionResetAtMs = sessionResetAt.getTime()
  const sessionResetCountdown = formatfreeportPremiumResetCountdown(
    sessionResetAt,
    now,
  )
  ...
  //   - top bar: paddingTop 1 + the Γ£ò row = 2
  ...
  // used to sit here too; it now rides inside the selector ΓÇö see below.)
  ...
  // warning itself stays visible either way ΓÇö scrollTop starts at 0, so it is
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      refreshfreeportLandingMetadata().catch(() => {})
    }, delayMs)
    ...
    >
      ...
      >
        {/* Empty spacer: justifyContent space-between needs a left sibling to
            keep the Γ£ò pushed to the right. */}
        <box />
        <Button
          onClick={() => exitCliCleanly()}
      ...
      >
        ...
        >
          ...
          {!session && !failure && (
            <text style={{ fg: theme.muted }}>
              <ShimmerText text="ConnectingΓÇª" />
            </text>
          )}
          ...
          {/* Country outside the free-mode allowlist. Terminal ΓÇö polling has
          ...
          {session?.status === 'country_blocked' && (
            <>
              <text style={{ fg: theme.secondary, marginBottom: 1 }}>
                ΓÜá Free mode isn't available in your region
              </text>
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                {session.countryBlockReason === 'anonymous_network' ? (
                  <>
                    We detected{' '}
                    {formatfreeportHardBlockedPrivacySignals(
                    ...
                    . FREEPORT can't be used from VPN, proxy, or Tor traffic.
                    Disable it and restart FREEPORT to try again.
                  </>
                ) : session.countryCode === 'UNKNOWN' ? (
                  <>
                    We couldn't verify an eligible location for this request.
                    VPN, Tor, proxy, or unknown-location traffic can't use
                    FREEPORT. Press Ctrl+C to exit.
                  </>
                ) : (
                  <>
                    We detected your location as{' '}
                    <span fg={theme.foreground}>{session.countryCode}</span>,
                    which is outside the countries where FREEPORT is currently
          ...
          {/* Shared free-session quota exhausted. Terminal for this run ΓÇö
              the user can exit and return after its daily or weekly reset. */}
          {session?.status === 'rate_limited' && (
            <>
              <text style={{ fg: theme.secondary, marginBottom: 1 }}>
                ΓÜá Session limit reached
              </text>
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                You've used{' '}
          ...
          {session?.status === 'spend_limited' && (
            <>
              <text style={{ fg: theme.secondary, marginBottom: 1 }}>
                Γÿò Daily FREEPORT limit reached
              </text>
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                ...
                {' ΓÇö '}your free usage resets automatically at midnight Pacific.
                Press Ctrl+C to exit.
              </text>
            </>
          )}
          ...
          {/* Too many distinct users hold a free session on this egress IP.
              Unlike the quota screens above there is no reset time ΓÇö a slot
              frees as soon as any session on the IP ends ΓÇö so this offers a
              short retry rather than a "come back tomorrow". */}
          {session?.status === 'ip_capped' && (
            <>
              <text style={{ fg: theme.secondary, marginBottom: 1 }}>
                ≡ƒÜª Too many FREEPORT sessions on this network
              </text>
              <text style={{ fg: theme.muted, wrapMode: 'word' }}>
                {session.activeUsersForIp} other people are already using
                FREEPORT from your network, which is the most we allow at once.
                Try again in{' '}
                <span fg={theme.foreground}>
                  {formatRetryAfter(session.retryAfterMs)}
                </span>
                {' ΓÇö '}a slot opens as soon as one of them finishes. Press
                Ctrl+C to exit.
              </text>
            </>
          )}
      ...
      {showAds && (
        ...
        >
          {ads ? (
            ...
            <text style={{ fg: theme.muted }}>{'ΓöÇ'.repeat(terminalWidth)}</text>
          )}
        </box>
      )}
    </box>
