import {
  FREEPORT_EARN_PATH,
  FREEPORT_EARN_PROMPT_SHORT,
} from '@codebuff/common/constants/freeport-levels'
...
import { REFERRAL_CLI_DAILY_SESSION_BONUS_CAP } from '@codebuff/common/constants/freeport-referral-tiers'
 ...
 * the slowest of the three ways to earn ΓÇö a referral pays when somebody else
 * signs up and sticks around. `/earn/trust` is the engagement feed, which pays
 * within a minute. `FREEPORT_EARN_PATH` is shared so the CLI, Desktop and the
 * browser all point at the same place.
 */
const EARN_URL = `${LOGIN_WEBSITE_URL}${FREEPORT_EARN_PATH}`
const DASHBOARD_LABEL = `${FREEPORT_EARN_PROMPT_SHORT} Γå╡`
...
function BountyPromoLine({
  ...
  promo?: freeportGlmPromo
}) {
  if (!promo) return null
  return (
    <text style={{ wrapMode: 'word' }}>
      <span fg={theme.success ?? theme.foreground}>
        Γ£ª Promo: earn a bounty, spend up to {promo.dailySessions} a day
      </span>
...
const COPY_FOCUS_ID = '__FREEPORT_referral_copy__'
const GLM_FOCUS_ID = '__FREEPORT_referral_glm__'
/** Opens the Earn page ΓÇö bounties to claim, referrals to track, balance to
 *  read. The CLI can open a browser (`safeOpen`), so this is a real button
 *  rather than a URL the user has to select and paste. */
const DASHBOARD_FOCUS_ID = '__FREEPORT_referral_dashboard__'
const BUTTON_HORIZONTAL_CHROME = 6 // two border + four padding columns

export interface freeportReferralFocusTarget {
  id: string
  activate: () => void
}
...
const shouldStackfreeportReferralActions = (width: number): boolean =>
  width < 70
 ...
 * "Γ£ö Copied!" confirmation for a couple seconds after a successful copy in
 ...
 * model picker.
 *
 *   - 'bordered' ΓÇö a rounded box that reads as a button. Used inside the
 *     unlocked GLM card, where it sits beside the "Use GLM 5.2 Γå╡" button and
 *     has to match it.
 *   - 'inline' ΓÇö no box; focus/hover shown by color alone, exactly like the
 ...
 *     whitespace, not a border, separates it from the pitch above ΓÇö and rests
...
const CopyInviteLinkButton: React.FC<{
  ...
  onCopy,
  availableWidth,
  labels = ['ΓÄÿ Copy invite link', 'ΓÄÿ Copy link', 'ΓÄÿ Copy'],
  variant = 'bordered',
}) => {
 ...
 * reward ΓÇö and the presentation ΓÇö depends on the session's access tier:
 *
 *   - LIMITED tier: referrals earn a daily free-session bonus (not GLM). One
 *     quiet muted line ("refer friends ΓåÆ more sessions per day") + the inline
 ...
 *     bordered card with your remaining sessions and a prominent "Use GLM 5.2 Γå╡"
 ...
 * card, so their copy control is borderless ΓÇö the hero card stays the only
...
export const FreeportReferralBanner: React.FC<FreeportReferralBannerProps> = ({
  ...
  const useGlm = useCallback(() => {
    ...
    setJoining(true)
    startfreeportSession(FREEPORT_GLM_V52_MODEL_ID).finally(() => {
  ...
  // A limited-tier user can now hold GLM sessions too ΓÇö bounty grants are
  // redeemable in every region ΓÇö so the unlocked card is keyed on the balance
  ...
  // quiet ΓÇö one line advertising the perk + the share button below it, with the
    ...
    return (
      <box
        style={{
          ...
          gap: 0,
          marginTop: 1,
          // Never let a height-starved landing column squash the banner ΓÇö that
          // would draw the copy control on top of the pitch line above it.
          flexShrink: 0,
        }}
      >
        <text style={{ wrapMode: 'word' }}>
          <span fg={theme.muted}>Γ£ª </span>
          {qualifiedCount > 0 ? (
            <>
  ...
  const resetsIn = formatfreeportPremiumResetCountdown(
  ...
  // NOT USABLE: keep it quiet ΓÇö one line that advertises the reward, with the
  // share link as a clearly-clickable button below it. Message adapts to *why*
  // it's locked ΓÇö no referrals yet vs. today's sessions already spent.
      ...
      >
        <text style={{ wrapMode: 'word' }}>
          ...
          {qualifiedCount > 0 ? (
            ...
            <>
              <span fg={theme.muted}>Refer friends ΓåÆ </span>
              <span fg={theme.foreground}>GLM 5.2</span>
  ...
  // whole sessions for a clean count ΓÇö an early-ended session leaves a fraction
  // that the user can still spend, so never show 0 here.
  const sessionsLeft = Math.max(1, Math.ceil(weeklySessionsRemaining))
  const stackActions = shouldStackfreeportReferralActions(width)
  ...
  const inviteLabels = [
    `ΓÄÿ Invite for +1/day (${qualifiedCount} earned)`,
    'ΓÄÿ Invite +1/day',
    'ΓÄÿ Invite',
  ]
  const githubLabel =
    actionRowWidth >=
    'Signed up with Google? Connect GitHub to qualify Γåù'.length
      ? 'Signed up with Google? Connect GitHub to qualify Γåù'
      : 'Connect GitHub to qualify Γåù'
  ...
  return (
    <box
      style={{
        ...
        marginTop: 1,
        width,
        // Never let a height-starved landing column squash the card ΓÇö that
        // would draw the bordered action buttons on top of the status line.
        flexShrink: 0,
      }}
      border={['top', 'bottom', 'left', 'right']}
      title=" Γ£ª GLM 5.2 unlocked "
      titleAlignment="left"
    >
      <text style={{ wrapMode: 'word' }}>
      ...
      >
        <Button
          ...
          // keyboard-focused, brighter on hover ΓÇö same scheme as the
