// The CLI can offer two kinds of FREEPORT row: the picker grid, and the referral banner's earned
// GLM 5.2 action. Both end up as a POST the server gates, and as a free-mode root agent that has
// to allow the model ΓÇö so a row this surface can show must survive all of it. Desktop shipped the
// mirror-image of this bug (an offered GLM row its own route answered 400 for), which is what
// these lock down here.

import { describe, expect, test } from 'bun:test'

import { getfreeportRootAgentIdForModel } from '@codebuff/common/constants/free-agents'
import {
  FREEPORT_GLM_V52_MODEL_ID,
  resolvefreeportModelForAccessTier,
} from '@codebuff/common/constants/freeport-models'
import { freeportOfferViolations } from '@codebuff/common/testing/freeport-offer-invariants'

import { freeportCliOfferedModelIds } from '../freeport-model-selector'

describe('FREEPORT rows the CLI offers', () => {
  for (const accessTier of ['full', 'limited'] as const) {
    test(`are all usable on the ${accessTier} tier`, () => {
      expect(
        freeportOfferViolations({
          surface: `cli picker + referral banner (${accessTier})`,
          accessTier,
          offered: freeportCliOfferedModelIds(accessTier),
          ...
          accepts: (model) =>
            resolvefreeportModelForAccessTier(model, accessTier) === model,
          rootAgentIdFor: getfreeportRootAgentIdForModel,
          catalog: 'supported',
        }),
      ).toEqual([])
    })
  ...
  test('the earned reward is offered on BOTH tiers, and the grid never shows it', () => {
    ...
    expect(freeportCliOfferedModelIds('full')).toContain(FREEPORT_GLM_V52_MODEL_ID)
    expect(freeportCliOfferedModelIds('limited')).toContain(
      FREEPORT_GLM_V52_MODEL_ID,
    )
  })
  ...
  test('the reward maps to its own root agent rather than the fallback', () => {
    expect(getfreeportRootAgentIdForModel(FREEPORT_GLM_V52_MODEL_ID)).toBe(
      'base2-free-glm',
    )
  })
})
