import { FREEPORT_EARN_PROMPT_SHORT } from '@codebuff/common/constants/freeport-levels'
...
afterEach(() => {
  ...
  usefreeportSessionStore.getState().setSession(null)
  usefreeportSessionStore.getState().setFailure(null)
  usefreeportModelStore.getState().setSelectedModel(FALLBACK_FREEPORT_MODEL_ID)
})
...
const renderSelector = async (maxHeight = 40) => {
  ...
  // earlier root mounted ΓÇö and a mounted selector keeps running its landing
...
const renderSelectorWithGlmRemaining = async (remaining?: number) => {
  ...
  usefreeportModelStore.getState().setSelectedModel(FREEPORT_GLM_V52_MODEL_ID)
...
describe('FreeportModelSelector referral selection', () => {
  ...
  test('still repairs a locked GLM selection to a visible grid model', async () => {
    ...
    expect(isfreeportModelId(getSelectedfreeportModel())).toBe(true)
  })
...
describe('FreeportModelSelector tier layout', () => {
  test('keeps the referral actions on one condensed row', async () => {
    ...
    usefreeportModelStore
      .getState()
      .setSelectedModel(FREEPORT_MINIMAX_M3_MODEL_ID)
    ...
    // (FREEPORT_EARN_PROMPT_SHORT), so asserting the constant rather than the
    // string keeps the three surfaces free to be re-worded together ΓÇö which is
    // the whole reason it is shared. What this test is really pinning is that
    // it sits on the SAME row as the copy control.
    expect(actionRow).toContain(FREEPORT_EARN_PROMPT_SHORT)
  ...
  test('orders Luna above MiniMax while keeping the saved premium model focused', async () => {
    ...
    usefreeportModelStore
      .getState()
      .setSelectedModel(FREEPORT_GPT_5_6_LUNA_MODEL_ID)
    ...
    // header rather than above it ΓÇö the premium pool is spent on the rows that
    // actually cost money.
    ...
    expect(frame).toContain('ΓÇ║ GPT-5.6 Luna')
    expect(frame).not.toContain('ΓÇ║ MiniMax M3')
  })
  ...
  test('shows the switch-to-Flash nudge only on the row the user is on', async () => {
    ...
    // still render on ONE line ΓÇö the width math reserves exactly its length.
    const notice = getfreeportModelSupersededBy(
      FREEPORT_MINIMAX_M3_MODEL_ID,
      FREEPORT_MODELS.map((m) => m.id),
    )!.notice
    ...
    // nags ΓÇö otherwise the list would repeat the same notice on every row it
    ...
    usefreeportModelStore
      .getState()
      .setSelectedModel(FREEPORT_MIMO_V25_MODEL_ID)
    ...
    usefreeportModelStore
      .getState()
      .setSelectedModel(FREEPORT_DEEPSEEK_V4_FLASH_MODEL_ID)
  ...
  test('places the exhausted-quota recommendation beneath UNLIMITED', async () => {
    ...
    // The unlimited recommendation is MiMo 2.5 since 2026-08-18 ΓÇö Flash moved
  ...
  test('collapses to the unlimited hero when the premium default is spent', async () => {
    ...
    // it, or Enter does nothing with no explanation ΓÇö and the picker has to
    ...
    usefreeportModelStore.getState().setSelectedModel(DEFAULT_FREEPORT_MODEL_ID)
    ...
    expect(getSelectedfreeportModel()).toBe(FALLBACK_FREEPORT_MODEL_ID)
    const frame = setup.captureCharFrame()
    // `ΓÇ║` is the cursor: it has to be on the row Enter now commits.
    expect(frame).toContain('ΓÇ║ MiMo 2.5')
    // ΓÇªand that row is the whole screen, exactly as for a user who is already
  ...
  test('repairs an invalid selection to the unlimited recommendation when premium is exhausted', async () => {
    ...
    expect(getSelectedfreeportModel()).toBe(FALLBACK_FREEPORT_MODEL_ID)
    expect(setup.captureCharFrame()).toContain('ΓÇ║ MiMo 2.5')
  })
  ...
  test('shows every limited-tier model when the access tier arrives after mount', async () => {
    ...
    usefreeportModelStore
      .getState()
      .setSelectedModel(FREEPORT_DEEPSEEK_V4_PRO_MODEL_ID)
    const setup = await renderSelector()
    ...
    for (const model of LIMITED_FREEPORT_MODELS) {
  ...
  test('badges only natively multimodal rows with Images', async () => {
    ...
    // description), but badging them made the label mean nothing ΓÇö and the
  ...
  test('sizes the hero card to its content, with no Press-Enter gutter', async () => {
    ...
    const heroRow = (
      frame.split('\n').find((line) => line.includes('ΓÇ║ DeepSeek V4 Flash')) ??
      ''
    ).trimEnd()
    ...
    expect(heroRow.endsWith('Γöé')).toBe(true)
...
describe('FreeportModelSelector limited-model offer', () => {
  ...
  test('renders the offered model with its scarcity and data-use label', async () => {
    usefreeportSessionStore.getState().setSession(offerSession())
    const frame = (await renderSelector()).captureCharFrame()
  ...
  test('keeps an offered selection instead of repairing it away', async () => {
    // The offer model is not in FREEPORT_MODELS, so the picker's
    // invalid-selection repair would otherwise bounce the user off the row they
    // just picked.
    usefreeportSessionStore.getState().setSession(offerSession())
    usefreeportModelStore.getState().setSelectedModel(FREEPORT_FABLE_5_MODEL_ID)
    await renderSelector()
    expect(getSelectedfreeportModel()).toBe(FREEPORT_FABLE_5_MODEL_ID)
  })
