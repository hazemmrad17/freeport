...
import { LandingHeadingRow } from '../freeport-landing-screen'
import { initializeThemeStore } from '../../hooks/use-theme'
import { getfreeportStreakLine } from '../../utils/freeport-streak-line'
 ...
 * is what broke the row ΓÇö with nothing wider on screen the column collapses
    ...
    root.render(
      <box style={{ flexDirection: 'column', alignItems: 'center', maxWidth: contentMaxWidth }}>
        <box style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 0 }}>
          <LandingHeadingRow
            streakLine={getfreeportStreakLine(streak)}
            marginBottom={1}
          />
        </box>
      </box>,
...
describe('LandingHeadingRow', () => {
  ...
  test('draws the progress dots as filled/hollow circles', async () => {
    ...
    expect(frame).toContain('ΓùÅΓùÅΓùÅΓùÅΓùÅΓùÅΓùÅ+')
  })
  ...
  // that overflow segfaulted the native renderer ΓÇö it has to survive as text.
