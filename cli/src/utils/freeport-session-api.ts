...
import {
  FREEPORT_COMPACT_SESSION_HEADER,
  FREEPORT_INSTANCE_HEADER,
  FREEPORT_MODEL_HEADER,
...
export type freeportSessionMethod = 'POST' | 'GET' | 'DELETE'

export class freeportSessionRequestError extends Error {
  constructor(
    message: string,
    ...
    super(message)
    this.name = 'freeportSessionRequestError'
  }
}

export function isfreeportSessionTimeoutError(error: unknown): boolean {
  return (
...
export type freeportSessionFailureDisposition = 'retry' | 'stop' | 'unknown'
...
export function classifyfreeportSessionRequestFailure(
  method: Extract<freeportSessionMethod, 'POST' | 'GET'>,
  error: unknown,
): freeportSessionFailureDisposition {
  if (method === 'POST') {
    if (!(error instanceof freeportSessionRequestError)) return 'unknown'
  ...
  if (!(error instanceof freeportSessionRequestError)) return 'retry'
...
function sessionEndpoint(): string {
  ...
  return `${base}/api/v1/FREEPORT/session`
}

export async function callfreeportSession(
  method: freeportSessionMethod,
  token: string,
  opts: {
    instanceId?: string
    model?: string
...
): Promise<freeportSessionServerResponse> {
  const headers: Record<string, string> = { Authorization: `Bearer ${token}` }
  if (method === 'GET' && opts.instanceId) {
    headers[FREEPORT_INSTANCE_HEADER] = opts.instanceId
  }
  if (method === 'GET' && opts.compact) {
    headers[FREEPORT_COMPACT_SESSION_HEADER] = '1'
  }
  if (method === 'POST' && opts.model) {
    headers[FREEPORT_MODEL_HEADER] = opts.model
  }
  ...
  if (response.status === 403) {
    const body = (await response
      .json()
      .catch(() => null)) as freeportSessionServerResponse | null
    if (
      body &&
  ...
  if (!response.ok) {
    ...
    throw new freeportSessionRequestError(
      `FREEPORT session ${method} failed: ${response.status} ${text.slice(0, 200)}`,
      response.status,
  ...
  return (await response.json()) as freeportSessionServerResponse
}
...
export function mergeCompactActiveSession(
  ...
  next: freeportSessionServerResponse,
): freeportSessionServerResponse | null {
  if (
...
export function holdsLivefreeportSlot(
  current: freeportSessionResponse | null,
): boolean {
...
export async function releasefreeportSlot(): Promise<void> {
  const current = usefreeportSessionStore.getState().session
  if (!holdsLivefreeportSlot(current)) return
...
/** Release the FREEPORT slot on exit paths that skip React unmount. */
export async function endfreeportSessionBestEffort(): Promise<void> {
  if (!IS_FREEPORT) return
  await releasefreeportSlot()
}
