...
Model traffic is routed through cost-optimized inference providers (DeepInfra, OpenRouter, Fireworks, CrofAI). The backend handles auth, sessions, ads, and usage logging.

## Models

Routed through cost-optimized providers (direct, not through the backend). We use multiple cheap inference providers — not just one vendor — to keep costs down and availability high:

| Model | Access |
|---|---|
| DeepSeek V4 Flash | Default, full access |
| GPT-5.6 Luna | Premium, multimodal |
| Muse Spark 1.2 | Premium, 1M context |
| Kimi K3 | Premium, via CrofAI |
| Claude Fable 5 | Premium, Anthropic trial |
...
- **CLI**: React + OpenTUI
- **Models**: DeepSeek V4, GPT-5.6 Luna, MiMo 2.5, MiniMax M3, Kimi K3, Claude Fable 5, Muse Spark 1.2, GLM 5.2 (via DeepInfra, OpenRouter, Fireworks, CrofAI)
