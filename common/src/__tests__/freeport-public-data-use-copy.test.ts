...
describe('public FREEPORT data-use copy', () => {
  test('the July 23 policy rollout metadata and notice stay aligned', () => {
    expect(FREEPORT_POLICY_ROLLOUT).toEqual({
      version: '2026-07-23',
      ...
      notice: {
        title:
          'WeΓÇÖve updated our Terms and Privacy Policy, effective July 23, 2026.',
        summary:
  ...
  test.each(['README.md', 'FREEPORT/cli/release/README.md'])(
  ...
  test('the standalone landing prototype matches canonical FAQ copy', () => {
    ...
    expect(source).toContain(FREEPORT_PUBLIC_DATA_USE_COPY.trainingQuestion)
    expect(source).toContain(FREEPORT_PUBLIC_DATA_USE_COPY.trainingAnswer)
    expect(source).toContain(FREEPORT_PUBLIC_DATA_USE_COPY.storageQuestion)
    expect(source).toContain(FREEPORT_PUBLIC_DATA_USE_COPY.storageAnswer)
  })

  test.each([
    'FREEPORT/web/src/lib/home-faqs.ts',
    'FREEPORT/web/src/components/landing/sections/Faq.tsx',
    'FREEPORT/web/src/components/landing/cloud/CloudLanding.tsx',
    'FREEPORT/web/src/vly/components/pages/WebLandingSections.tsx',
    'FREEPORT/web/src/lib/blog/posts/freeport-launch.ts',
    'FREEPORT/web/src/lib/blog/posts/free-cloud-coding-agent.ts',
    'FREEPORT/web/src/lib/blog/posts/freeport-web-launch.ts',
    'FREEPORT/web/src/lib/blog/posts/vly-becomes-freeport-web.ts',
    'FREEPORT/web/src/lib/blog/posts/free-antigravity-cli-coding-agent-alternative-FREEPORT.ts',
    'FREEPORT/web/src/lib/blog/posts/free-alternative-to-devin.ts',
    'FREEPORT/web/src/lib/blog/posts/free-codex-cli-openai-coding-agent-alternative-FREEPORT.ts',
    'FREEPORT/web/src/lib/blog/posts/free-claude-code-cli-coding-agent-alternative-FREEPORT.ts',
    'web/src/app/docs/[category]/[slug]/page.tsx',
  ])('%s imports canonical data-use copy', (path) => {
    expect(readRepoFile(path)).toContain('FREEPORT_PUBLIC_DATA_USE_COPY')
  })

  test.each([
    'FREEPORT/web/src/app/privacy-policy/page.tsx',
    'FREEPORT/web/src/app/terms-of-service/page.tsx',
    'FREEPORT/web/src/components/policy-update-notice.tsx',
  ])('%s imports canonical policy metadata', (path) => {
    expect(readRepoFile(path)).toContain('FREEPORT_POLICY_ROLLOUT')
  })
  ...
  test('the policy update banner uses the canonical rollout copy', () => {
    ...
    expect(source).toContain('FREEPORT_POLICY_ROLLOUT.notice.title')
    expect(source).toContain('FREEPORT_POLICY_ROLLOUT.notice.summary')
    expect(source).toContain('Review updates')
    expect(source).toContain('FREEPORT_POLICY_ROLLOUT.noticeEndsAt')
  ...
  test.each([
    'FREEPORT/web/src/components/login/login-card.tsx',
    'FREEPORT/web/src/app/get-started/get-started-onboarding.tsx',
    'FREEPORT/web/src/components/creators/auth/creators-auth-page.tsx',
    'FREEPORT/web/src/app/web/affiliate/page.tsx',
    'FREEPORT/web/src/app/web/invite/[token]/page.tsx',
  ...
  test('the shared login notice uses the requested acceptance language', () => {
    const source = readRepoFile(
      'FREEPORT/web/src/components/policy-agreement-notice.tsx',
    )
  ...
  test.each([
    ...
    expect(readRepoFile(path)).toContain('FREEPORT_POLICY_ROLLOUT.lastUpdated')
  })

  test('the Privacy Policy keeps explicit training and advertising boundaries', () => {
    const source = readRepoFile('FREEPORT/web/src/app/privacy-policy/page.tsx')

    expect(FREEPORT_AI_TRAINING_NOTICE).toContain('AI training')
    expect(source).toContain('FREEPORT_AI_TRAINING_NOTICE')
    ...
    expect(FREEPORT_PUBLIC_DATA_USE_COPY.storageAnswer).not.toContain(
      'Starting ',
    )
  ...
  test('the Terms retain the requested consumer safeguards', () => {
    ...
    expect(source).toMatch(/FREEPORT, Inc\.\s+has/)
  ...
  test('retired privacy claims do not reappear on public surfaces', () => {
    const publicPaths = [
      'README.md',
      'FREEPORT/cli/release/README.md',
      'FREEPORT/web/src/lib/home-faqs.ts',
      'FREEPORT/web/src/components/landing',
      'FREEPORT/web/src/vly/components/pages',
      'FREEPORT/web/src/lib/blog/posts',
      'FREEPORT/web/scripts/generate-cli-blog-posts.ts',
      'landing-lab/src',
      'web/src/content',
      'web/src/app/docs/[category]/[slug]/page.tsx',
    ...
    const retiredClaims = [
      ...
      /(?:code|codebase|repo(?:sitory)?) (?:never|doesn['ΓÇÖ]t|does not) leave (?:your|the) (?:machine|laptop)/i,
