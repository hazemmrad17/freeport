...
} from '../constants/freeport-peak-hours'
import { resolvefreeportSpendCeiling } from '../constants/freeport-spend-ceilings'
...
describe('the peak windows', () => {
  test.each([
    ...
    [3, true],
    [4, false],
    [5, false], // the gap BETWEEN the two windows ΓÇö what a single range check
...
describe('nextDeepSeekWindowBoundary', () => {
  ...
  test.each([
    ['mid-peak, to that windowΓÇÖs close', 2, at(4)],
    ['mid-second-window, to its close', 7, at(10)],
    ['in the gap, to the next windowΓÇÖs OPEN', 4, at(6)],
    ['long off-peak, wrapping to tomorrowΓÇÖs 01:00', 11, new Date(at(1).getTime() + DAY)],
...
describe('formatDeepSeekPeakWindowsLocal', () => {
  test('renders both windows in the readerΓÇÖs zone, not UTC', () => {
...
describe('the peak-hours ceiling reduction', () => {
  ...
  test('does nothing without an explicit opt-in', () => {
    ...
    const plain = resolvefreeportSpendCeiling(base)
    const timeOnly = resolvefreeportSpendCeiling({ ...base, at: at(2) })
  ...
  test('halves the ceiling during peak and says when it returns', () => {
    const full = resolvefreeportSpendCeiling(base)
    const peak = resolvefreeportSpendCeiling({
      ...base,
      at: at(2),
      peakMultiplier: 0.5,
    })
    ...
    // 04:00 ΓÇö hours away, NOT at midnight Pacific.
    expect(peak.peak?.endsAt.getTime()).toBe(at(4).getTime())
  })

  test('leaves off-peak hours untouched', () => {
    const offPeak = resolvefreeportSpendCeiling({
      ...base,
      at: at(12),
      peakMultiplier: 0.5,
    })
    expect(offPeak.usd).toBe(resolvefreeportSpendCeiling(base).usd)
  ...
  test('reduces the WINNER, so peak cannot change which rule binds', () => {
    ...
    // back at 04:00 ΓÇö an account would change refusal reasons twice a day for
    // no policy reason.
    const restricted = {
    ...
    const peak = resolvefreeportSpendCeiling(restricted)
    const offPeak = resolvefreeportSpendCeiling({ ...restricted, at: at(12) })
  ...
  test.each([0, -1, Number.NaN, 1, 2])(
    ...
    (multiplier) => {
      ...
      const r = resolvefreeportSpendCeiling({
        ...base,
        at: at(2),
        peakMultiplier: multiplier,
      })
      expect(r.usd).toBe(resolvefreeportSpendCeiling(base).usd)
      expect(r.peak).toBeUndefined()
    },
  )

  test('leaves the limited tier alone ΓÇö it runs no DeepSeek model', () => {
    ...
    // charge a limited account for a rate card it cannot reach ΓÇö so this test
    ...
    expect(peak.usd).toBe(resolvefreeportSpendCeiling(limited).usd)
