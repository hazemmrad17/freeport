/**
 * Kimi K3 (Eco) is god-only on FREEPORT Web — a testing row, not a product one.
...
const KIMI_ID = FREEPORT_KIMI_K3_ECO_MODEL_ID

describe('Kimi K3 is god-only on FREEPORT Web', () => {
  it('is offered to god users and nobody else', () => {
    expect(isfreeportWebGodOnlyModelId(KIMI_ID)).toBe(true)
    expect(isfreeportWebModelId(KIMI_ID, { includeGodOnly: true })).toBe(true)
    expect(isfreeportWebModelId(KIMI_ID, { includeGodOnly: false })).toBe(false)
    expect(FREEPORT_WEB_GOD_ONLY_MODELS.map((m) => m.id)).toContain(KIMI_ID)
    // The god-only list is additive to the visible one, so it must NOT also
    // appear there or every user would see it.
    expect(FREEPORT_WEB_MODELS.map((m) => m.id)).not.toContain(KIMI_ID)
  })

  it('stays off every non-web surface', () => {
    // Web only, as asked. FREEPORT_MODELS is the CLI/Desktop catalog and
    // SUPPORTED_FREEPORT_MODELS the waiting-room set.
    expect(FREEPORT_MODELS.map((m) => m.id)).not.toContain(KIMI_ID)
    expect(SUPPORTED_FREEPORT_MODELS.map((m) => m.id)).not.toContain(KIMI_ID)
    expect(FREEPORT_WEB_ALL_MODELS.map((m) => m.id)).toContain(KIMI_ID)
  })
  ...
  it('is metered by the premium pool, never the standard one', () => {
    ...
    expect(FREEPORT_WEB_PREMIUM_MODEL_IDS).toContain(KIMI_ID)
    expect(FREEPORT_STANDARD_MODEL_IDS).not.toContain(KIMI_ID)
    const model = FREEPORT_WEB_GOD_ONLY_MODELS.find((m) => m.id === KIMI_ID)
  ...
  it('keeps its id distinct from every other catalog row', () => {
    ...
    const all = FREEPORT_WEB_ALL_MODELS.map((m) => m.id)
