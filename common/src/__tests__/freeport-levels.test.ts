...
import {
  FREEPORT_LEVELS,
  FREEPORT_LEVEL_0,
  FREEPORT_LEVEL_SESSION_CEILING,
  FREEPORT_MAX_LEVEL,
  FREEPORT_TRUST_COST_PER_PROMPT,
  FREEPORT_TRUST_ALLOW_NEGATIVE,
  FREEPORT_TRUST_MIN_BALANCE,
  levelForTrust,
  levelProgress,
  levelSessionBonus,
  nextLevelAfter,
} from '../constants/freeport-levels'
import * as freeportModels from '../constants/freeport-models'
import {
  FREEPORT_LIMITED_SESSION_LIMIT,
  FREEPORT_PREMIUM_SESSION_LIMIT,
  FREEPORT_PRE_LEVELS_LIMITED_SESSION_LIMIT,
  FREEPORT_PRE_LEVELS_PREMIUM_SESSION_LIMIT,
...
describe('the ladder', () => {
  it('is strictly increasing in every axis that costs us money', () => {
    ...
    for (let i = 1; i < FREEPORT_LEVELS.length; i++) {
      const lower = FREEPORT_LEVELS[i - 1]!
      const higher = FREEPORT_LEVELS[i]!
  ...
  it('starts exactly where the flat session bases start', () => {
    ...
    // half-done base reduction ΓÇö see "Lowering the base, later" in
    // freeport-levels.ts, which lists the four edits that must land together.
    //
    // `freeSessionsPerDay` is the LIMITED-region pool, not the full-access
    // standard one ΓÇö that pool is deliberately not on this ladder at all.
    expect(FREEPORT_LEVEL_0.trustRequired).toBe(0)
    expect(FREEPORT_LEVEL_0.freeSessionsPerDay).toBe(
      FREEPORT_LIMITED_SESSION_LIMIT,
    )
    expect(FREEPORT_LEVEL_0.premiumSessionsPerDay).toBe(
      FREEPORT_PREMIUM_SESSION_LIMIT,
    )
  })

  it('does not reintroduce a full-access standard limit', () => {
    // The floor of the product is UNMETERED on every surface ΓÇö the browser-only
    ...
    expect(freeportModels).not.toHaveProperty(
      'FREEPORT_WEB_STANDARD_SESSION_LIMIT',
    )
    expect(freeportModels).not.toHaveProperty('FREEPORT_STANDARD_SESSION_LIMIT')
  })

  it('tops out at the ceiling the copy promises, on both pools', () => {
    expect(FREEPORT_MAX_LEVEL.freeSessionsPerDay).toBe(
      FREEPORT_LEVEL_SESSION_CEILING,
    )
    expect(FREEPORT_MAX_LEVEL.premiumSessionsPerDay).toBe(
      FREEPORT_LEVEL_SESSION_CEILING,
    )
    for (const tier of FREEPORT_LEVELS) {
  ...
  it('never takes a session away from where the base starts', () => {
    ...
    for (const tier of FREEPORT_LEVELS) {
      expect(tier.freeSessionsPerDay).toBeGreaterThanOrEqual(
        FREEPORT_LEVEL_0.freeSessionsPerDay,
      )
      expect(tier.premiumSessionsPerDay).toBeGreaterThanOrEqual(
        FREEPORT_LEVEL_0.premiumSessionsPerDay,
      )
    }
  })
  ...
  it('climbs past what the pools paid BEFORE the reduction', () => {
    ...
    expect(FREEPORT_MAX_LEVEL.premiumSessionsPerDay).toBeGreaterThan(
      FREEPORT_PRE_LEVELS_PREMIUM_SESSION_LIMIT,
    )
    expect(FREEPORT_MAX_LEVEL.freeSessionsPerDay).toBeGreaterThan(
      FREEPORT_PRE_LEVELS_LIMITED_SESSION_LIMIT,
    )
  })

  it('gets a limited-region account back above the old base within a few levels', () => {
    // The cut that actually stings is 6 ΓåÆ 3. This pins how much work undoing
    // it costs: if a re-tune pushes it past a handful of engagements, the
    // reduction has stopped being a nudge and become a wall.
    const recovered = FREEPORT_LEVELS.find(
      (tier) =>
        tier.freeSessionsPerDay >= FREEPORT_PRE_LEVELS_LIMITED_SESSION_LIMIT,
    )
...
describe('levelForTrust', () => {
  ...
  it('clamps a negative balance to level 0', () => {
    // Spending below zero is allowed on purpose (FREEPORT_TRUST_ALLOW_NEGATIVE)
    // and "you are level -1" is not a state any surface should have to render.
    expect(levelForTrust(-1).level).toBe(0)
    expect(levelForTrust(FREEPORT_TRUST_MIN_BALANCE).level).toBe(0)
  })

  it('tops out rather than running off the end', () => {
    expect(levelForTrust(FREEPORT_MAX_LEVEL.trustRequired * 10).level).toBe(
      FREEPORT_MAX_LEVEL.level,
    )
    expect(nextLevelAfter(FREEPORT_MAX_LEVEL.level)).toBeNull()
  })
})
...
describe('levelProgress', () => {
  it('measures from the CURRENT level floor, not from zero', () => {
    ...
    const second = FREEPORT_LEVELS[2]!
    ...
    const almostThere = levelProgress(FREEPORT_LEVELS[3]!.trustRequired - 1)
    expect(almostThere.level).toBe(2)
    expect(almostThere.progress).toBeGreaterThan(0.95)
  })

  it('is full and terminal at the top rung', () => {
    const top = levelProgress(FREEPORT_MAX_LEVEL.trustRequired)
  ...
  it('reports the remaining work in engagements, not just points', () => {
    ...
    expect(zero.engagementsToNextLevel).toBe(
      Math.ceil(
        FREEPORT_LEVELS[1]!.trustRequired / FREEPORT_LEVEL_0.trustPerEngagement,
      ),
    )
  ...
  it('never reports negative work remaining', () => {
    const below = levelProgress(FREEPORT_TRUST_MIN_BALANCE)
...
describe('prompt costs', () => {
  it('decays slower than it is earned, at every level', () => {
    ...
    for (const tier of FREEPORT_LEVELS) {
      const premiumPrompts =
        tier.trustPerEngagement / FREEPORT_TRUST_COST_PER_PROMPT.premium
  ...
  it('never prices a scarcer prompt below a cheaper one', () => {
    ...
    // keeping is only that it never INVERTS ΓÇö a frontier prompt must not cost
    // less than a standard one if the classes are ever split again.
    expect(FREEPORT_TRUST_COST_PER_PROMPT.frontier).toBeGreaterThanOrEqual(
      FREEPORT_TRUST_COST_PER_PROMPT.premium,
    )
    expect(FREEPORT_TRUST_COST_PER_PROMPT.premium).toBeGreaterThanOrEqual(
      FREEPORT_TRUST_COST_PER_PROMPT.standard,
    )
  })
  ...
  it('charges exactly 1 for a message, whatever it was sent to', () => {
    ...
    // score moved. Flat 1 is a rate a person can reason about ΓÇö an engagement
    // pays 50, so it covers 50 messages.
    for (const cost of Object.values(FREEPORT_TRUST_COST_PER_PROMPT)) {
      expect(cost).toBe(1)
    }
  })
  ...
  it('never lets a balance go below zero', () => {
    ...
    expect(FREEPORT_TRUST_MIN_BALANCE).toBe(0)
    expect(FREEPORT_TRUST_ALLOW_NEGATIVE).toBe(false)
  })
