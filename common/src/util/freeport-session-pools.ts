 ...
 * FREEPORT Web took the first Premium row with a quota. That held until
 ...
 * is the point ΓÇö the next model to get its own ceiling is a server edit, and
...
import type { freeportSessionRateLimit } from '../types/freeport-session'

export interface freeportSectionQuotas {
  /** The quota the section HEADER should show, or undefined when the section
   *  has no metered rows at all. */
  header: freeportSessionRateLimit | undefined
  ...
  perModel: Record<string, freeportSessionRateLimit>
}
 ...
 * in display order. Rows with no quota (unmetered models) are ignored ΓÇö they
 * belong to no pool and need no label.
 */
export function getfreeportSectionQuotas(
  models: readonly string[],
  quotas: Record<string, freeportSessionRateLimit> | undefined,
): freeportSectionQuotas {
  if (!quotas) return { header: undefined, perModel: {} }

  const rows = models
    .map((model) => quotas[model])
    .filter((quota): quota is freeportSessionRateLimit => Boolean(quota))
  if (rows.length === 0) return { header: undefined, perModel: {} }

  // Older servers send no `pool` at all. Falling back to one bucket reproduces
  // the previous behaviour exactly ΓÇö header from the first row, nothing inline
  // ΓÇö so a new client against an old server is no worse than before.
  const poolOf = (quota: freeportSessionRateLimit) => quota.pool ?? ''
  ...
  const perModel: Record<string, freeportSessionRateLimit> = {}
  for (const quota of rows) {
...
/**
 * "1 of 1 used" ΓÇö the inline chip for a row on its own pool.
 *
 ...
 */
export function formatfreeportRowQuota(
  quota: freeportSessionRateLimit,
): string {
