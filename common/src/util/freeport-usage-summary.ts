import { FREEPORT_USAGE_MAP_DAYS } from '../constants/freeport-models'

import {
  addDaysToDateKey,
  calculatefreeportStreak,
  FREEPORT_STREAK_TIME_ZONE,
 ...
 */
export function calculateLongestfreeportStreak(
...
export function buildFREEPORTUsageSummary(params: {
  ...
  const { streak, todayUsed, lastUsageDate } = calculatefreeportStreak({
    usageDates: allDates,
    todayDateKey,
  })

  return {
    timeZone: params.timeZone ?? FREEPORT_STREAK_TIME_ZONE,
    todayDateKey,
    streak: {
      current: streak,
      longest: calculateLongestfreeportStreak(allDates),
      todayUsed,
      lastUsageDate,
    },
 ...
 * today. Callers render `null` as empty space, not as a quiet day ΓÇö those days
 * either predate the window or have not happened yet.
 */
export function buildfreeportUsageGrid(params: {
  activeDates: readonly string[]
  todayDateKey: string
  windowDays: number
