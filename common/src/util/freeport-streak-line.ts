import { FREEPORT_STREAK_REWARDS_ENABLED } from '../constants/freeport-models'
import {
  getfreeportStreakGlmWeeklyUnits,
  isfreeportStreakGlmBonusActive,
} from './freeport-streak'

/** Days in a streak "week" ΓÇö the milestone the progress dots fill toward. */
export const FREEPORT_STREAK_WEEK = 7

export interface freeportStreakLine {
  /** Count label, e.g. "2 day streak". */
  label: string
  /** A week's worth of progress dots toward the 7-day milestone, e.g.
   *  "ΓùÅΓùÅΓùïΓùïΓùïΓùïΓùï". Fills to "ΓùÅΓùÅΓùÅΓùÅΓùÅΓùÅΓùÅ" at 7, then gains a trailing "+"
   *  ("ΓùÅΓùÅΓùÅΓùÅΓùÅΓùÅΓùÅ+") for any streak beyond the week so long runs read as
   ...
   *  FREEPORT_STREAK_WEEK, carried so a renderer that can't import this module
...
export interface freeportStreakDotChars {
  filled: string
  empty: string
}
 ...
 * entirely ΓÇö new / lapsed users should be nudged to start using the product,
 * not shown an empty streak.
 *
 * The glyphs default to ΓùÅ/Γùï, which is what a real UI font renders best; a
 * surface whose font can't be trusted with those passes its own pair.
 */
export function getfreeportStreakLine(
  streak: number,
  chars: freeportStreakDotChars = { filled: 'ΓùÅ', empty: 'Γùï' },
): freeportStreakLine | null {
  if (streak <= 0) return null

  // Fill toward the 7-day milestone, then stay full ΓÇö a 19-day streak should
  ...
  const filled = Math.min(streak, FREEPORT_STREAK_WEEK)
  const beyond = streak > FREEPORT_STREAK_WEEK
  const dots =
    chars.filled.repeat(filled) +
    chars.empty.repeat(FREEPORT_STREAK_WEEK - filled) +
    (beyond ? '+' : '')

  // "day" stays singular ΓÇö it's a compound modifier ("7 day streak"), not a
  // count of days on its own.
  return {
    label: `${streak} day streak`,
    dots,
    progress: { filled, total: FREEPORT_STREAK_WEEK, beyond },
  }
}

/**
 * A short perk note for an active streak. Below the 7-day milestone it teases
 * the countdown ("N more days to unlock ΓÇª") so the reward motivates the users
 ...
 * too ΓÇö a lapsed user needs a first day, not a countdown from seven).
 *
 * The daily-pool bonus (+1 session) recurs **every day** the streak stays at 7+,
 * so it's framed as "every day". The GLM 5.2 bonus refills with the GLM pool ΓÇö
 * daily since 2026-07-29 (weekly before) ΓÇö while the streak remains active and
 ...
 */
export function getfreeportStreakBonusNote(params: {
  streak: number
  ...
  const includesGlm =
    params.accessTier === 'full' && isfreeportStreakGlmBonusActive()
  ...
  const glmDaily = Math.max(1, getfreeportStreakGlmWeeklyUnits(params.streak))
  ...
  if (params.streak < FREEPORT_STREAK_WEEK) {
    const remaining = FREEPORT_STREAK_WEEK - params.streak
    return `≡ƒÄü ${remaining} more ${remaining === 1 ? 'day' : 'days'} to unlock ${perk}`
  }
  return `≡ƒÄü Streak perk: ${perk}`
}
