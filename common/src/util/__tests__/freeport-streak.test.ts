...
import {
  ...
  getfreeportDailyStreakRewardPool,
  getfreeportStreakGlmBonusUnits,
  getfreeportUsageDateKey,
...
describe('FREEPORT streak helpers', () => {
  test('formats usage dates in the FREEPORT reset timezone', () => {
    expect(getfreeportUsageDateKey(new Date('2026-05-27T06:30:00.000Z'))).toBe(
      '2026-05-26',
    )
    expect(getfreeportUsageDateKey(new Date('2026-05-27T08:30:00.000Z'))).toBe(
      '2026-05-27',
    )
  })
...
describe('FREEPORT streak rewards', () => {
  ...
  test('no daily reward below seven days or before today is used', () => {
    ...
    // FREEPORT today, so no bonus is earned yet.
    expect(
      getfreeportDailyStreakRewardPool({
        streak: 7,
        todayUsed: false,
