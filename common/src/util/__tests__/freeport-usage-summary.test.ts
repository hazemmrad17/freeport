...
describe('calculateLongestfreeportStreak', () => {
  ...
  it('does not extend a run across a gap, unlike the current streak', () => {
    ...
    expect(calculateLongestfreeportStreak(['2026-08-01', '2026-08-03'])).toBe(1)
  })
  ...
  it('is zero with no history', () => {
    expect(calculateLongestfreeportStreak([])).toBe(0)
  })
...
describe('buildfreeportUsageGrid', () => {
  ...
  it('pads to whole weeks so every row is one weekday', () => {
    const grid = buildfreeportUsageGrid({
      activeDates: [],
      todayDateKey,
      windowDays: 365,
      todayWeekday,
  ...
  it('places today at its own weekday in the final column', () => {
    ...
    // Everything after today in the final column is padding, not a quiet day ΓÇö
