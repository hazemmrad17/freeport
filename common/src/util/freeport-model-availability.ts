 ...
 * "blocked") ΓÇö clear enough to answer the question for someone who goes
 ...
 * surfaces cannot drift ΓÇö a user who reads it in one and asks support about the
 ...
 * that is now gone ΓÇö unlike every other notice, its subject is not on screen.
 *
 * Kept to three clauses because it wraps under the picker, and delete it when
 * Flash returns to LIMITED_FREEPORT_MODEL_IDS.
 */
export const FREEPORT_PAUSED_MODEL_NOTICE =
  "DeepSeek V4 Flash 07/31 is paused here after a steep price increase ΓÇö pausing it is what keeps these sessions free for everyone. We're working to bring it back."

/**
 * The limits, in three clauses and a signature.
 *
 * SHORT ON PURPOSE. This replaced a version that listed every rule ΓÇö per-model
 * caps, peak hours, quantization, which pool each row spends ΓÇö and was four
 ...
 */
export const FREEPORT_TIER_CHANGE_NOTICE =
  'DeepSeek costs have spiked, so limits are tighter for now: V4 Pro and GPT-5.6 Luna are 1 session a day, and V4 Pro pauses at peak times. MiMo 2.5 and MiniMax M3 stay unlimited. ΓÇöFREEPORT Team'

const PRIVACY_SIGNAL_LABELS: Partial<Record<freeportIpPrivacySignal, string>> =
  {
...
export function formatfreeportPrivacySignalList(
...
/** "BR" ΓåÆ "Brazil". Falls back to the raw code when the runtime can't
 *  resolve it (malformed code, missing ICU data). */
export function formatfreeportCountryName(countryCode: string): string {
  try {
    return (
 ...
 *
 * A missing/unknown reason still returns a line ΓÇö the reduced catalog is
 * visible either way, so saying nothing is the one option that leaves the
 * question unanswered.
 */
export function getfreeportModelAvailabilityNotice(
  reason: freeportLimitedModeReason | null | undefined,
): string {
  ...
  switch (reason.countryBlockReason) {
    case 'anonymous_network':
      return `Using a ${formatfreeportPrivacySignalList(
    ...
    case 'country_not_allowed':
      return `Some models aren't available in ${
        countryCode ? formatfreeportCountryName(countryCode) : 'your region'
      } yet`
