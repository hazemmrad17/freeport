 ...
 * FREEPORT model row, every gate that row then passes through must accept it.**
 *
 * FREEPORT model ids fan out across several hand-kept lists ΓÇö the per-tier
 ...
 * offerΓåÆgate direction the user sees a row they cannot use: FREEPORT Desktop
 * shipped the earned GLM 5.2 row while its `/api/thread/:id/agent` route still
 * validated against `FREEPORT_MODELS` (which excludes GLM by design), so every
 * user who earned the reward got 400 "invalid model" when they picked it.
 *
 * Drift the OTHER way is fine and deliberate ΓÇö an id that is acceptable but not
 * offered is exactly how a staged retirement works ΓÇö so this only ever checks
 * offered Γèå accepted.
 *
 * Each surface owns its offer set (they live in the desktop/cli/web packages,
 * which `common` cannot import), so each calls `freeportOfferViolations` from
...
export type freeportCatalogSource = 'supported' | 'web'

export interface freeportOfferSurface {
  /** Names the surface in failure messages, e.g. 'freeport-desktop picker (full)'. */
  surface: string
  /** The tier whose rows `offered` describes ΓÇö gates are tier-sensitive. */
   ...
   *  `isModelForHarness`, the web picker's selectable filter, ΓÇª). */
  ...
  catalog?: freeportCatalogSource
}

function rootAgentIds(
  surface: freeportOfferSurface,
  model: string,
): readonly string[] {
...
function violationsForModel(
  ...
  if (!isfreeportSessionModelId(model)) {
    // everything below is derived from the same catalogs, so one unknown id
    // would otherwise produce five near-identical lines
    return [`${where} is offered but is not a FREEPORT session model id`]
  }

  // 2. ΓÇªand admit it for THIS tier, or admission answers model_not_allowed
  if (!isfreeportSessionModelAllowedForAccessTier(model, surface.accessTier)) {
    out.push(
  ...
  const resolved = resolvefreeportSessionModelForAccessTier(
    model,
    surface.accessTier,
  )
  if (resolved !== model) {
    out.push(
      `${where} is offered but resolvefreeportSessionModelForAccessTier coerces it to ${resolved}`,
    )
  }

  // 4. the surface's own validator ΓÇö the check the GLM 400 needed
  ...
  for (const rootId of roots) {
    ...
    if (!FREEPORT_ROOT_AGENT_IDS.some((id) => id === rootId)) {
      out.push(
        `${where} runs under free-mode root ${rootId}, which is missing from FREEPORT_ROOT_AGENT_IDS (its subagents would 403)`,
      )
    }
  }
  ...
  if (surface.catalog) {
    const row =
      surface.catalog === 'web'
        ? getfreeportWebModel(model)
        : getfreeportModel(model)
    if (row.id !== model) {
      out.push(
...
/** Every offerΓåÆgate violation on a surface. Assert `toEqual([])`: the strings
 *  are the failure message. */
export function freeportOfferViolations(
  surface: freeportOfferSurface,
): string[] {
  ...
  if (surface.offered.length === 0) {
    return [`${surface.surface}: offers no models at all ΓÇö check the test wiring`]
  }
