/**
 * FREEPORT Levels — the earned half of the free-capacity story.
 *
 * ## Why this exists, next to `FREEPORT-trust.ts`
 *
 * `FREEPORT-trust.ts` ("Access Level") answers *how much do we believe this
 ...
 * selected. That invariant is the same one `FREEPORT-trust.ts` documents, and
 ...
 * being drained (see `FREEPORT_LIMITED_SESSION_LIMIT`). Everything above that
 ...
 *
 * `FREEPORT_LEVEL_SESSIONS=off` reverts the reduced bases AND suppresses the
 ...
 * was removed in the same week (see the note on `FREEPORT_STANDARD_MODEL_IDS`).
 * That is the floor of the product, the thing we mean when we say FREEPORT is
 ...
 *
 *   - every number lives in ONE table (`FREEPORT_LEVELS`) rather than being
...
export interface FREEPORTLevelTier {
 ...
 * cost a bounded amount — the same reasoning `FREEPORT-trust.ts` gives for
 * `core` being ~1.6x `established` rather than unbounded.
 */
export const FREEPORT_LEVEL_SESSION_CEILING = 7
 ...
 */
export const FREEPORT_LEVELS: readonly FREEPORTLevelTier[] = [
  {
    level: 0,
    name: 'Newcomer',
    trustRequired: 0,
...
export const FREEPORT_LEVEL_0 = FREEPORT_LEVELS[0]!
export const FREEPORT_MAX_LEVEL = FREEPORT_LEVELS[FREEPORT_LEVELS.length - 1]!
 ...
 * below zero — see `FREEPORT_TRUST_ALLOW_NEGATIVE` — and "you are level −1" is
 * not a state any surface should have to render.
 */
export function levelForTrust(trust: number): FREEPORTLevelTier {
  let held = FREEPORT_LEVEL_0
...
export function nextLevelAfter(level: number): FREEPORTLevelTier | null {
  return FREEPORT_LEVELS.find((tier) => tier.level === level + 1) ?? null
}
 ...
 */
export interface FREEPORTLevelProgress {
  trust: number
  level: number
  levelName: string
  trustPerEngagement: number
...
export function levelProgress(trust: number): FREEPORTLevelProgress {
...
export function levelSessionBonus(level: number): {
  ...
  const tier =
    FREEPORT_LEVELS.find((entry) => entry.level === level) ?? FREEPORT_LEVEL_0
  return {
    free: Math.max(0, tier.freeSessionsPerDay - FREEPORT_LEVEL_0.freeSessionsPerDay),
    premium: Math.max(
      0,
      tier.premiumSessionsPerDay - FREEPORT_LEVEL_0.premiumSessionsPerDay,
    ),
  }
}
 ...
 */
export const FREEPORT_TRUST_COST_PER_PROMPT = {
   ...
   *  inline; see `FREEPORT-trust-buffer.ts`. `enqueuePromptTrustCharge` still
...
export type FREEPORTTrustCostClass = keyof typeof FREEPORT_TRUST_COST_PER_PROMPT
 ...
 */
export const FREEPORT_TRUST_ALLOW_NEGATIVE = false
 ...
 */
export const FREEPORT_TRUST_MIN_BALANCE = 0

// ---------------------------------------------------------------------------
// Copy
// ---------------------------------------------------------------------------

export const FREEPORT_TRUST_CURRENCY_NAME = 'Trust'
 ...
 */
export const FREEPORT_EARN_PROMPT = `Earn ${FREEPORT_TRUST_CURRENCY_NAME} · level up for more daily sessions`

/** Shorter form, for a row that already has a quota summary beside it. */
export const FREEPORT_EARN_PROMPT_SHORT = `Earn ${FREEPORT_TRUST_CURRENCY_NAME} for more sessions`

/** Relative to the FREEPORT web app. The Trust subroute, not the Earn index:
 *  it is the tab that pays today, and the index only redirects here. */
export const FREEPORT_EARN_PATH = '/earn/trust'

/** Shown under the progress bar. Indexed by level, falling back to the last. */
export const FREEPORT_LEVEL_BLURBS: Record<number, string> = {
...
export function levelBlurb(level: number): string {
  return FREEPORT_LEVEL_BLURBS[level] ?? FREEPORT_LEVEL_BLURBS[10]!
}
