/**
 * FREEPORT account standing ("Access Level") ΓÇö the per-account policy layer
 ...
 * are scarce, and `docs/freeport-signup-gate.md` is the record of them not
 ...
 *
 * `freeportAccessTier` (full / limited) is a REGION property, resolved per
 * request from the caller's IP country. `freeportTrustLevel` is an ACCOUNT
 ...
 * and get more than a full-region `new` account ΓÇö which is the whole point of
 ...
 * a NUMBER ΓÇö a limit ΓÇö and a limit that is reached produces a retryable 429
 * naming the remedy. `docs/freeport-abuse-detection.md` records what it cost
 ...
 * `FREEPORT_TRUST_LEVEL_LABELS` is the one bridge between the two.
 */

import type { freeportAccessTier } from './freeport-models'
...
export type freeportTrustLevel = (typeof FREEPORT_TRUST_LEVELS)[number]

/** The level an account holds before anything is known about it. Every failure
 *  path in the resolver must land somewhere DEFINITE, and this is not it ΓÇö see
 *  `FREEPORT_TRUST_FALLBACK_LEVEL`. */
export const FREEPORT_TRUST_MIN_LEVEL: freeportTrustLevel = 'new'
 ...
 */
export const FREEPORT_TRUST_FALLBACK_LEVEL: freeportTrustLevel = 'established'

export function isAtLeastTrustLevel(
  level: freeportTrustLevel,
  minimum: freeportTrustLevel,
): boolean {
  return (
    FREEPORT_TRUST_LEVELS.indexOf(level) >=
    FREEPORT_TRUST_LEVELS.indexOf(minimum)
  )
}

function lowerOf(
  a: freeportTrustLevel,
  b: freeportTrustLevel,
): freeportTrustLevel {
  return isAtLeastTrustLevel(a, b) ? b : a
}

/** User-facing name. Never says "trust", "risk" or "score" ΓÇö a user reading
 *  their own level is reading an explanation of their limits, not a verdict on
 *  their character. */
export const FREEPORT_TRUST_LEVEL_LABELS: Record<freeportTrustLevel, string> = {
  new: 'Getting started',
  verified: 'Verified',
...
export const FREEPORT_TRUST_LEVEL_BLURBS: Record<freeportTrustLevel, string> = {
  new: 'Welcome! Your account is brand new, so limits start small. They open up quickly ΓÇö the steps below take a few minutes.',
  verified:
    'Your account is verified. You have solid daily limits, and a bit of history unlocks the next level.',
  established:
    'You are an established FREEPORT user with generous limits on messages, spend and premium sessions.',
 ...
 * budget and none is a concurrency cap ΓÇö read each doc comment rather than
 * inferring from the name.
 */
export interface freeportTrustLimits {
  /**
  ...
  /** Total free-mode LLM requests per day. The overall ceiling ΓÇö a premium
 ...
 * `FREEPORT_PREMIUM_SESSION_LIMIT` as it always has, and `core` came down from
 * 10 to 5.
 *
 * `core` was doing two jobs. It was the abuse control's verdict ΓÇö this account
 * is demonstrably real ΓÇö and it was also the only reward in the product big
 ...
 *
 * The reward half moved to `freeport-levels.ts`, which is denominated in
 * something a user can go and do right now, and which tops out at
 * `FREEPORT_LEVEL_SESSION_CEILING` (7) ΓÇö above every value in this column. So
 ...
 *
 * This column only selects anything under `FREEPORT_TRUST_LEVELS=enforce`;
 * under the default `observe` the flat base applies, so these numbers and the
 * `FREEPORT_LEVEL_SESSIONS` revert do not interact.
 *
 ...
 *
 * **Concurrent Desktop tabs** (`FREEPORT_DESKTOP_SESSION_LIMITS.unlimited`) are
 ...
 * away from exactly the users we most need to keep ΓÇö a new user discovers "you
 * may only open one tab" instantly, and it reads as a product that is broken
 * rather than as a budget ΓÇö in exchange for no measurable saving at all. The
 ...
 *
 * `established` ├ù `full` is the current flat-limit fallback (5,000/day,
 * 3,000/5h, $50, 5 premium), and `established` ├ù `limited` reproduces the
 ...
 * not an exemption ΓÇö an account that reaches `core` and is then compromised or
 ...
 *
 * Its floor is deliberately harsher (`new` ├ù limited is a third of `new` ├ù
 * full) because that intersection ΓÇö brand-new account, unsupported region,
 * often VPN ΓÇö is the exact shape of the reselling farms. Its ceiling is
 * deliberately generous (`core` ├ù limited beats `verified` ├ù full on every
 * axis it can ΓÇö premium is region-gated, not level-gated) because the entire
 * promise this makes to a real developer in an unsupported country is that the
 * region is a starting point and not a cage.
 */
export const FREEPORT_TRUST_LIMITS: Record<
  freeportAccessTier,
  Record<freeportTrustLevel, freeportTrustLimits>
> = {
  full: {
    new: {
...
export function freeportTrustLimits(
  accessTier: freeportAccessTier,
  level: freeportTrustLevel,
): freeportTrustLimits {
  return FREEPORT_TRUST_LIMITS[accessTier][level]
}
 ...
 * account created before `docs/freeport-signup-gate.md` shipped has null
 ...
 * clean slate. Unknown earns nothing and costs nothing ΓÇö which lands those
 * accounts at whatever their other, positive signals justify.
 */
export interface freeportTrustSignals {
  /** `user.created_at`. */
  accountCreatedAt: Date | null
  /** `referral_qualification.github_account_created_at` ΓÇö set by GitHub
   ...
   *  (`FREEPORT_daily_usage`). Cheap history that cannot be bought. */
  activeDays: number
  /** Approved bounty submissions. Reviewed proof-of-work ΓÇö the single
  ...
  /** `user.privacy_flagged_at` ΓÇö first request ever seen on an ipinfo-flagged
  ...
  /** `user.privacy_corroborated_at` ΓÇö first request where a second provider
   *  agreed the egress was anonymizing. Sticky. */
  privacyCorroboratedAt: Date | null
  /** `user.third_party_client_at` ΓÇö first free-mode request carrying a tool
   *  schema no FREEPORT client ships. Sticky, and behavioural rather than
   ...
   * change in a second by toggling a VPN ΓÇö which is why it can only CAP a
...
export interface freeportTrustFactor {
  id: string
  label: string
  points: number
}
...
export interface freeportTrustNextStep {
  id: string
  label: string
  detail: string
  points: number
  /** Where the UI should send them. Relative to the FREEPORT web app. */
  href?: string
}

export interface freeportTrustAssessment {
  level: freeportTrustLevel
   ...
   *  cap applied ΓÇö which is how the UI knows to explain the cap rather than
   *  telling someone with 80 points to keep earning points. */
  uncappedLevel: freeportTrustLevel
  /** Which cap bound, if any. */
  cappedBy: string | null
  factors: freeportTrustFactor[]
  nextSteps: freeportTrustNextStep[]
}
 ...
 * ΓÇö bounties, referrals, 90 days of account age, 30 active days, a clean
 * residential signup ΓÇö topped out at **70**. Five points short. `core` was
 ...
 * incentive flat exactly where it should be steep ΓÇö a user who completed ten
 ...
 * (docs/freeport-bounties.md). A qualified referral requires the REFERRED
 ...
 * (docs/freeport-abuse-referral-farming.md). An operator who can produce ten
 * qualified referrals has already cleared a higher bar than this scorer sets.
 */
export const FREEPORT_TRUST_EARNED = {
  BOUNTY_POINTS: 5,
  BOUNTY_CAP: 6,
  REFERRAL_POINTS: 3,
  REFERRAL_CAP: 10,
...
const MAX_REFERRAL_POINTS =
  FREEPORT_TRUST_EARNED.REFERRAL_POINTS * FREEPORT_TRUST_EARNED.REFERRAL_CAP

/** Minimum score for each level. `new` is the floor and needs no entry. */
export const FREEPORT_TRUST_THRESHOLDS: Record<
  Exclude<freeportTrustLevel, 'new'>,
  number
> = {
  verified: 25,
  established: 50,
...
function levelForScore(score: number): freeportTrustLevel {
  if (score >= FREEPORT_TRUST_THRESHOLDS.core) return 'core'
  if (score >= FREEPORT_TRUST_THRESHOLDS.established) return 'established'
  if (score >= FREEPORT_TRUST_THRESHOLDS.verified) return 'verified'
  return 'new'
}
 ...
 *
 * Positive signals add points. Negative signals mostly do NOT subtract ΓÇö they
 ...
 * do. A ceiling is not, and it also degrades honestly ΓÇö a VPN user who cannot
 * exceed `established` still gets `established`, which is today's limits.
 */
export function assessfreeportTrust(
  signals: freeportTrustSignals,
  now: Date = new Date(),
): freeportTrustAssessment {
  const factors: freeportTrustFactor[] = []
  const nextSteps: freeportTrustNextStep[] = []

  const add = (id: string, label: string, points: number) => {
    if (points === 0) return
    factors.push({ id, label, points })
  }
  const step = (s: freeportTrustNextStep) => nextSteps.push(s)
  ...
  if (githubAge === null) {
    ...
    if (githubAge >= 3 * YEAR_MS) {
      ...
      step({
        ...
        detail:
          'Account age is worth up to 20 points and grows on its own ΓÇö nothing to do here but keep the same account connected.',
        points: 10,
      })
    }
    ...
    if (signals.githubTwoFactorEnabled) {
      ...
      step({
        ...
        detail:
          'Worth 5 points, and it protects the account your FREEPORT limits now depend on.',
        points: 5,
  ...
  if (accountAge !== null) {
    if (accountAge >= 90 * DAY_MS) {
      add('account_age', 'FREEPORT account over 90 days old', 15)
    } else if (accountAge >= 30 * DAY_MS) {
      add('account_age', 'FREEPORT account over 30 days old', 10)
    } else if (accountAge >= 7 * DAY_MS) {
      add('account_age', 'FREEPORT account over 7 days old', 5)
    }
  }

  if (signals.activeDays >= 30) {
    add('active_days', 'Used FREEPORT on 30+ days', 10)
  } else if (signals.activeDays >= 7) {
    add('active_days', 'Used FREEPORT on 7+ days', 5)
  }
  ...
  // a half.
  const bountyPoints =
    Math.min(signals.approvedBounties, FREEPORT_TRUST_EARNED.BOUNTY_CAP) *
  ...
  if (bountyPoints < MAX_BOUNTY_POINTS) {
    step({
      ...
      detail: `Approved bounties are worth ${FREEPORT_TRUST_EARNED.BOUNTY_POINTS} points each, up to ${MAX_BOUNTY_POINTS}. They are reviewed, they work from any country, and they pay session grants on top.`,
  ...
  const referralPoints =
    Math.min(signals.qualifiedReferrals, FREEPORT_TRUST_EARNED.REFERRAL_CAP) *
  ...
  if (referralPoints < MAX_REFERRAL_POINTS) {
    step({
      ...
      detail: `Each friend who signs up with a real GitHub account and uses FREEPORT is worth ${FREEPORT_TRUST_EARNED.REFERRAL_POINTS} points, up to ${MAX_REFERRAL_POINTS} ΓÇö plus the referral rewards themselves.`,
  ...
  if (signals.hasPaid) {
    add('paid', 'Supported FREEPORT with a purchase', 25)
  }
  ...
  const cap = (limit: freeportTrustLevel, reason: string) => {
  ...
  // A reversed ban is invisible here by construction ΓÇö only unreversed events
  // reach this field ΓÇö so this is an account we actioned and did not take
  ...
  // defeated by toggling the VPN off for a day ΓÇö the exact wash-trading of
  ...
  // the cap is prepended after the sort rather than competing in it ΓÇö it
 ...
 * action that does not exist ΓÇö a "next step" a user cannot take is worse than
 * no step at all.
 */
const CAP_REMEDIES: Record<string, { label: string; detail: string }> = {
  ...
  past_privacy_egress: {
    ...
    detail:
      'A connection from this account looked like an anonymizing network. That caps this account at Established. If this seems wrong ΓÇö some office and university networks are misread ΓÇö contact support.',
  },
  third_party_client: {
    label: 'A non-FREEPORT client has used this account',
    detail:
      'Requests from this account carried a client we do not ship. That caps this account at Verified. Only official FREEPORT apps are supported on free mode.',
  },
  anonymous_network: {
  ...
  signup_privacy_egress: {
    ...
    detail:
      'That caps this account at Established. Everything else still counts, and the cap applies to this account only ΓÇö it is not a strike against you.',
  },
 ...
 * to map level ΓåÆ limits itself would hold a second copy of the matrix above
 ...
 *
 * 1. **A published limit is a published target.** `docs/freeport-abuse-
 *    detection.md` records that the abuse pattern here is not bursting, it is
 *    "sustained pacing just under the daily caps" ΓÇö so telling an operator
 ...
 *    answer.
 *
 * So `freeportStandingInfo` carries these phrases and NOT `freeportTrustLimits`
 * ΓÇö the raw matrix never crosses the wire, which means no client can render it
 ...
 * disagree with it.
 */
export interface freeportStandingHighlight {
  label: string
  value: string
}
...
export function freeportStandingHighlights(
  accessTier: freeportAccessTier,
  level: freeportTrustLevel,
): freeportStandingHighlight[] {
  ...
  return [
    ...
    { label: 'Work per prompt', value: phrases.depth },
    {
      ...
      // buy it.
      value:
        freeportTrustLimits(accessTier, level).premiumSessionsPerDay > 0
          ? phrases.premium
 ...
 * what the account actually gets once `FREEPORT_TRUST_LEVELS=enforce`. Both
 ...
 * the same ΓÇö see the comment in FREEPORT/web/src/app/api/web/standing/route.ts
 * for what happens otherwise.
 */
export interface freeportStandingInfo {
  level: freeportTrustLevel
  label: string
  blurb: string
  score: number
  ...
  nextLevel: freeportTrustLevel | null
  cappedBy: string | null
  cappedReason: string | null
  factors: freeportTrustFactor[]
  nextSteps: freeportTrustNextStep[]
  accessTier: freeportAccessTier
  /** Semantic, never numeric ΓÇö see freeportStandingHighlight. */
  highlights: freeportStandingHighlight[]
}

export function tofreeportStandingInfo(
  assessment: freeportTrustAssessment,
  accessTier: freeportAccessTier,
): freeportStandingInfo {
  const index = FREEPORT_TRUST_LEVELS.indexOf(assessment.level)
  const nextLevel = FREEPORT_TRUST_LEVELS[index + 1] ?? null
  return {
    level: assessment.level,
    label: FREEPORT_TRUST_LEVEL_LABELS[assessment.level],
    blurb: FREEPORT_TRUST_LEVEL_BLURBS[assessment.level],
    score: assessment.score,
    nextLevel,
    nextLevelAt:
      nextLevel && nextLevel !== 'new'
        ? FREEPORT_TRUST_THRESHOLDS[nextLevel]
        : null,
    ...
    accessTier,
    highlights: freeportStandingHighlights(accessTier, assessment.level),
  }
}
