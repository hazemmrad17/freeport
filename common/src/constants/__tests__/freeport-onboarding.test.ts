...
describe('the question set itself', () => {
  it('gives every option a unique id within its question', () => {
    ...
    for (const q of FREEPORT_ONBOARDING_QUESTIONS) {
  ...
  it('maps every legacy option onto an option that still exists', () => {
    ...
    // supposed to rescue ΓÇö the exact failure the map exists to prevent.
    for (const [questionId, map] of Object.entries(ONBOARDING_LEGACY_OPTION_IDS)) {
      const question = FREEPORT_ONBOARDING_QUESTIONS.find((q) => q.id === questionId)
...
describe('classifyOnboardingOtherText ΓÇö write-ins folded into real options', () => {
...
describe('validateOnboardingSubmission', () => {
  ...
  it('requires every question', () => {
    ...
    // client rather than a user choice ΓÇö and half a response would sit in the
...
describe('isOnboardingComplete ΓÇö the blocking gate reads this', () => {
  ...
  it('treats null, undefined and empty as incomplete', () => {
    // A user with no record must be gated, not waved through ΓÇö this is the
