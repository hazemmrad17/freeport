 ...
 * `FREEPORT_daily_usage` (one narrow row per active Pacific day, which already
 * exists for the streak) and a short bounded aggregate over `message`. See
 * docs/FREEPORT-account-hub.md.
 */

export interface FREEPORTUsageStreakSummary {
 ...
 */
export interface FREEPORTRecentUsage {
...
export interface FREEPORTUsageSessionsByModel {
  model: string
  sessions: number
  units: number
}

export interface FREEPORTUsageSummary {
  timeZone: string
  ...
  todayDateKey: string
  streak: FREEPORTUsageStreakSummary
  /**
  ...
  sessionsByModel: FREEPORTUsageSessionsByModel[]
}
