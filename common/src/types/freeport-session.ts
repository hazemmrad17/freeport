import type { freeportAccessTier } from '../constants/freeport-models'
import type { freeportStandingInfo } from '../constants/freeport-trust'

/**
 * Wire-level shapes returned by `/api/v1/FREEPORT/session`. Source of truth
 * for the CLI (which deserializes these) and the server (which serializes
 * them) ΓÇö keep both in sync by importing this module from either side.
 *
 ...
 * joined model consumes FREEPORT sessions. `recentCount` is the
 * rounded session units since the current quota period began at the time the
 * response was produced ΓÇö see also the standalone `rate_limited` status for
 * the reject path.
 */
export interface freeportSessionEntitlementBreakdown {
   ...
   *  promo runs, which is the default ΓÇö an older client that never reads it
   *  still sums to the right `limit`, because the server sends the total. */
  promo?: number
  /**
   * Sessions added by the account's earned LEVEL
   * (`common/constants/freeport-levels.ts`). Omitted at level 0, which is
   * where every account starts, so the field is absent for most callers and an
   * older client that never reads it still sums to the right `limit` ΓÇö the
...
export interface freeportSessionRateLimit {
  model: string
  /**
   ...
   * and label the whole section from it ΓÇö the CLI literally took
   ...
   *
   * `pool` is an opaque token ΓÇö clients must GROUP by it and never match on its
  ...
  entitlementBreakdown?: freeportSessionEntitlementBreakdown
  limit: number
...
export type freeportSessionRateLimitByModel = Record<
  string,
  freeportSessionRateLimit
>

/** Timing needed by multi-session clients to show the active session window. */
export interface freeportActiveSessionInfo {
  model: string
  admittedAt: string
  expiresAt: string
}

/** Live per-user FREEPORT Desktop sessions, counted server-side across every
...
export interface freeportDesktopSessionCounts {
  premium: number
  unlimited: number
 ...
 *
 *   - full tier    ΓåÆ unlock daily GLM 5.2 sessions (`weeklySessionsRemaining`
 *     and `resetAt` carry the live balance / next reset; the field name
 *     predates the weeklyΓåÆdaily cadence change and is kept for wire compat).
 *   - limited tier ΓåÆ earn a daily free-session bonus (+1/day per qualified
 ...
 */
export interface freeportReferralInfo {
   ...
   *  landing page ("X invited you to try FREEPORT!"). Null when the user has no
   *  name set. */
  ...
  /** GLM sessions still available in the current reset window (entitlement ΓêÆ
   *  used, ΓëÑ 0). Daily since 2026-07-29; the "weekly" name is kept for wire
 ...
 */
export interface freeportGlmPromo {
...
): freeportGlmPromo | undefined =>
  session && 'glmPromo' in session
    ? ((session as { glmPromo?: freeportGlmPromo }).glmPromo ?? undefined)
    : undefined
...
export const getReferralInfo = (
  ...
  session && 'referral' in session
    ? (session as { referral?: freeportReferralInfo }).referral
    : undefined
 ...
 *  loose so the CLI can pass its `freeportSessionResponse` (which adds the
...
export const getRateLimitsByModel = (
  session: { status: string } | null | undefined,
): freeportSessionRateLimitByModel | undefined =>
  session && 'rateLimitsByModel' in session
    ? (session as { rateLimitsByModel?: freeportSessionRateLimitByModel })
        .rateLimitsByModel
    : undefined
 ...
 * has sessions in it, so it ΓÇö not the client ΓÇö decides whether the row exists
 ...
 * as "not now" rather than hiding the row ΓÇö the user has already had their
 ...
 * (FREEPORT_LIMITED_OFFER_SESSION_LIMIT, which no streak or referral can
 ...
 */
export interface freeportLimitedModelOffer {
  /** Model id; always a member of SUPPORTED_FREEPORT_MODELS, so the client can
   *  resolve its display name from the shared catalog. */
  model: string
  /** Sessions left in the shared global pool for the current wave. Always > 0
   *  ΓÇö an exhausted offer is omitted rather than sent as zero. */
  remaining: number
...
export const getLimitedModelOffers = (
  session: { status: string } | null | undefined,
): freeportLimitedModelOffer[] =>
  session && 'limitedModelOffers' in session
    ? ((session as { limitedModelOffers?: freeportLimitedModelOffer[] })
        .limitedModelOffers ?? [])
    : []

export type freeportCountryBlockReason =
  | 'country_not_allowed'
...
export type freeportIpPrivacySignal =
  | 'anonymous'
  | 'vpn'
  | 'proxy'
  | 'tor'
...
export type freeportSpurStatus =
  | 'not_checked'
  | 'clean'
  | 'suspicious'
  | 'failed'
   ...
   * IP at all ΓÇö scoring it would penalise every request in the world the
...
export type freeportScamalyticsStatus =
  | 'not_checked'
  | 'clean'
  | 'suspicious'
  | 'failed'
  /** Deliberately not consulted ΓÇö switched off, or balance exhausted and the
   *  breaker is open. Same meaning and same reasoning as the Spur variant. */
  | 'skipped'

export type freeportPrivacyDecision =
  | 'allowed_clean'
  | 'ipinfo_suspicious_spur_clean'
  | 'corroborated_block'
  | 'cloudflare_tor_block'
  | 'spur_failed_limited'
  /** ipinfo flagged the egress and no second opinion was obtainable at all ΓÇö
...
export type freeportPrivacyProviderDecision =
  | 'not_checked'
  | 'cloudflare_tor'
  | 'ipinfo_clean'
  | 'ipinfo_failed'
...
export interface freeportLimitedModeReason {
  ...
  countryBlockReason?: freeportCountryBlockReason | null
  ipPrivacySignals?: freeportIpPrivacySignal[] | null
}

export type freeportSessionServerResponse = (
  | ({
       ...
       *  off ΓÇö see freeportLimitedModelOffer. */
      limitedModelOffers?: freeportLimitedModelOffer[]
      /**
       ...
       */
      standing?: freeportStandingInfo
    } & freeportLimitedModeReason)
  | ({
      status: 'active'
      accessTier: freeportAccessTier
      instanceId: string
      /** Model the active session is bound to ΓÇö cannot change mid-session. */
      model: string
      admittedAt: string
      expiresAt: string
      remainingMs: number
      /** Shared free-session quota for this model. */
      rateLimit?: freeportSessionRateLimit
      rateLimitsByModel?: freeportSessionRateLimitByModel
  ...
  | ({
       ...
       *  server-side grace window ΓÇö chat requests still go through so the
  ...
  | {
       ...
       *  their first chat request. Terminal ΓÇö
  ...
  | {
       ...
       *  your active DeepSeek session to switch?" ΓåÆ on confirm, DELETE then
  ...
  | {
       ...
       *  start a session at all. Terminal ΓÇö CLI stops polling and shows a
       *  banned message. */
      status: 'banned'
    }
  | {
      ...
      /** The configured ceiling (FREEPORT_IP_USER_CAP). */
      limit: number
      retryAfterMs: number
    }
  | {
      ...
      status: 'rate_limited'
      accessTier?: freeportAccessTier
      /** The FREEPORT model the user tried to join. */
      model: string
      ...
      windowHours: number
      /** Session units since the current Pacific period began ΓÇö will be ΓëÑ limit. */
      recentCount: number
      /** Milliseconds from now until the next Pacific period reset. */
      retryAfterMs: number
    }
  | {
      /** The user has reached today's cross-model FREEPORT provider-spend
       ...
       * at ..." instead of the flat "you are out for the day" ΓÇö which would be
  ...
  | {
      /** FREEPORT Desktop multi-session only: the user already holds an active
       ...
       *  the LIMITED tier every model occupies the slot ΓÇö one FREEPORT tab at
  ...
  desktopSessionCounts?: freeportDesktopSessionCounts
}
 ...
 * never by its `message` ΓÇö the prose is written for whoever is sitting in front
 ...
 * one of them has the same recovery ΓÇö forget the dead window and re-admit on
 * the same instance id ΓÇö which is why clients collapse them into one state
 ...
 * docs/freeport-session-admission.md.
 */
export const FREEPORT_GATE_CODES = {
 ...
 *  its `error` field, so `in` would accept inherited names like `toString` ΓÇö
 ...
 *  rejection. */
export function getfreeportGateCode(output: {
  error?: string | undefined
  statusCode?: number | undefined
}): freeportGateCode | null {
  const code = output.error
  if (!code || !Object.hasOwn(FREEPORT_GATE_CODES, code)) return null
  const gate = FREEPORT_GATE_CODES[code as freeportGateCode]
  return gate.status === output.statusCode ? (code as freeportGateCode) : null
}
